...
This repo includes a public key file for strongname signing. If you download the source and build it, your assemblies will be open source strongname signed, which is sufficient for other strongname signed assemblies to be able to call into them, but will not pass direct signing validation. Users are welcome to sign their own builds, or to cancel the strongname signing by editing the project files.
Microsoft will release fully Authenticode-signed versions of the tool and libraries soon, probably through existing Software Development Kits and possible other channels.

### Secrets
Do not add certificates, Azure secrets, private keys (including strongname private keys), or any other private or potentially sensitive information to the repo. If a secret is added even temporarily, it will be discoverable long after deletion and must be invalidated immediately. If you need certificates for testing, use functions in CoseSign1.Tests.Common.TestCertificateUtils to create them, or add your own functions to create custom certificates if those do not meet your needs.

### Certificate store access
The CoseHandler class has a static LookupCertificate method to find installed certificates. If you modify this method or add other methods that access the local certifiacte store, please ensure that your changes are compatible across Windows, Linux, and MacOS operating systems, as each have slightly different certificate store implementations.
When writing unit tests that touch store functions, avoid making any changes to the certificate stores on the local machine. This can be done to some extent through Moq, and more accurately by testing the lookup functions with existing certificates. If you must add a certificate to the local store, create a custom store by setting StoreName to a non-standard value, such as "TestStore", and consider making the test an integration test that only runs manually.
...
* Use concrete collecion types where possible. Many LINQ functions, such as .Any(), will do multiple type conversions when given an IEnumerable or other interface type.
* Use "Count > 0" instead of "Any()".

### Static methods
Use static methods instead of instance methods where possible. Instance methods carry the entire object instance with them in a "this" pointer, which takes up more throughput.

### Avoid dependency injection
Dependency injection, when it touches the product code, adds substantion memory and computational overhead and should be avoided at all costs. There should be no evidence of test code in the product code beyond the occasional constructor stub or interface, and remember that calling interfaces adds a type conversion step, so should be used sparingly.

## Functional considerations
### Streams
One of the main uses for COSE signing is to sign Software Bills of Materials (SBOM), which can sometimes exceed 2gb in size. When this happens, the file cannot fit into a byte array or memory stream because the count of bytes exceeds the maximum integer size. Instead, these need to be passed as Streams, or more specifically FileStreams. Note that only the payload file can exceed 2gb because the CoseSign1Message object has a backing byte array, and therefore when signing streams it only supports detached signing, whereas can sign either detached or embedded when given a byte array, and embed-signed files can never exceed 2gb.

## Unit tests
All new or changed functionality must have unit tests (or integration tests if unit testign is not feasible) to verify that it works as expected.

### Testing frameworks
The test in this solution use the standard MSTest framework provided by Visual Studio, plus Fluent Assertions for syntax and Moq for mocking. While there are many great test frameworks out there, we ask that contributors limit themselves to these to prevent code bloat and reduce cognitive load for other contributors.
...
For consistency, please use FluentAssertions syntax for asserts. _Link needed_

## API design
The CoseHandler project features a user-centric API, designed to make COSE signing and validation, and reading of COSE embed-signed files, as easy as possible. The user doesn't have to know anything beyond "I need to sign X file with Y certificate". It doesn't matter if their certificate is a .pfx file or a thumbprint, or if the content they want to sign is a file, stream, or byte array, it just works. And if it doesn't work, it will tell them why in a way that is easy for a lay person to understand and correct, such as "The signing certificate must include a private key."
...
If for any reason you have to break an existing public API, please flag the change so it gets a through review and so that breaking change notifications can go out before the change is published.

### Follow existing patterns
New methods and overloads added to CoseHandler should resemble the existing ones. Keep input parameters in the same order where possible, make sure you support file, stream, and byte array inputs, and support options for specifying certificates by thumbprint for signing operations.

## Coding style
### Full comment blocks for all public types and members
Each non-private type or member in a non-test project must have a full triple-slash comment block including a summary, and descriptions for all parameters, return value, and all possible exceptions, including those thrown by other members the current member calls into.
...
* Optional parameters should start with "Optional. " and include the default value if not null.
...
* ...any conditional block, loop structure, or LINQ query.
* ...any line that contains a ternary or null-coalescing operator.
* ...any code that has to do something unintuitive.

### Line breaks
* Parameters in a method decalration should be wrapped and indented if:
...
### Strings
Use interpolated strings with the $"Text {variable} more text" format. This is both more performant and easier to read than string.format, stringbuilders, or concatenation. Using "text " + "more text" to break lines during string initialization does not count as concatenating an existing string.
* If you cannot create the whole string all at once, try to modify it as few times as possible.
* Use @"" or $@"" syntax for multiline strings instead of "\r\n".
