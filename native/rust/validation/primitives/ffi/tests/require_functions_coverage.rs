...
fn test_require_cwt_claim_text_present_null_key() {
    ...
    let status = unsafe {
        cose_sign1_trust_policy_builder_require_cwt_claim_text_present(policy_builder, ptr::null())
    };
...
fn test_require_cwt_claim_label_i64_eq_null_builder() {
    let status = unsafe {
        cose_sign1_trust_policy_builder_require_cwt_claim_label_i64_eq(ptr::null_mut(), 1000, 42)
    };
    assert_ne!(status, cose_status_t::COSE_OK);
}
...
fn test_require_cwt_claim_label_bool_eq_null_builder() {
    let status = unsafe {
        cose_sign1_trust_policy_builder_require_cwt_claim_label_bool_eq(ptr::null_mut(), 1001, true)
    };
    assert_ne!(status, cose_status_t::COSE_OK);
}
...
fn test_require_cwt_exp_le_coverage() {
    ...
    let status =
        unsafe { cose_sign1_trust_policy_builder_require_cwt_exp_le(policy_builder, future) };
    assert_eq!(status, cose_status_t::COSE_OK);
...
fn test_comprehensive_claim_combinations() {
    ...
    let status = unsafe {
        cose_sign1_trust_policy_builder_require_cwt_claim_label_i64_ge(policy_builder, 1000, 50)
    };
    assert_eq!(status, cose_status_t::COSE_OK);

    let status = unsafe {
        cose_sign1_trust_policy_builder_require_cwt_claim_label_i64_le(policy_builder, 1001, 100)
    };
    assert_eq!(status, cose_status_t::COSE_OK);

    // Add boolean requirement
    let status = unsafe {
        cose_sign1_trust_policy_builder_require_cwt_claim_label_bool_eq(policy_builder, 1002, true)
    };
    assert_eq!(status, cose_status_t::COSE_OK);

    // Add timestamp requirements
    let status =
        unsafe { cose_sign1_trust_policy_builder_require_cwt_exp_ge(policy_builder, 1640995200) };
    assert_eq!(status, cose_status_t::COSE_OK);

    let status =
        unsafe { cose_sign1_trust_policy_builder_require_cwt_nbf_le(policy_builder, 1672531200) };
    assert_eq!(status, cose_status_t::COSE_OK);
