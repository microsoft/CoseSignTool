...
use cose_sign1_transparent_mst_ffi::{
    ...
    cose_mst_trust_options_t, cose_sign1_mst_trust_policy_builder_require_receipt_issuer_contains,
    ...
    cose_sign1_validator_builder_with_mst_pack, cose_sign1_validator_builder_with_mst_pack_ex,
    MstClientHandle,
};
...
fn with_mst_pack_ex_null_builder() {
    let status =
        cose_sign1_validator_builder_with_mst_pack_ex(std::ptr::null_mut(), std::ptr::null());
    assert_ne!(status, cose_status_t::COSE_OK);
}
...
fn with_mst_pack_ex_null_options() {
    let mut builder = make_builder();
    let status = cose_sign1_validator_builder_with_mst_pack_ex(&mut *builder, std::ptr::null());
    assert_eq!(status, cose_status_t::COSE_OK);
}
...
fn require_receipt_signature_not_verified() {
    let mut pb = make_policy_builder_with_mst();
    let status =
        cose_sign1_mst_trust_policy_builder_require_receipt_signature_not_verified(&mut *pb);
    assert_eq!(status, cose_status_t::COSE_OK);
}
...
fn require_issuer_eq() {
    ...
    let status =
        cose_sign1_mst_trust_policy_builder_require_receipt_issuer_eq(&mut *pb, issuer.as_ptr());
    assert_eq!(status, cose_status_t::COSE_OK);
}
...
fn require_kid_contains() {
    ...
    let status =
        cose_sign1_mst_trust_policy_builder_require_receipt_kid_contains(&mut *pb, needle.as_ptr());
    assert_eq!(status, cose_status_t::COSE_OK);
}
...
fn require_trusted_from_issuer_contains() {
    ...
    let status = cose_sign1_mst_trust_policy_builder_require_receipt_trusted_from_issuer_contains(
        &mut *pb,
        needle.as_ptr(),
    );
    assert_eq!(status, cose_status_t::COSE_OK);
