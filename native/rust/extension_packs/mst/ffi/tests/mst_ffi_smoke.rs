...
fn policy_require_receipt_present() {
    let p = make_policy();
    assert_eq!(
        cose_sign1_mst_trust_policy_builder_require_receipt_present(p),
        cose_status_t::COSE_OK
    );
}
...
fn policy_require_receipt_not_present() {
    let p = make_policy();
    assert_eq!(
        cose_sign1_mst_trust_policy_builder_require_receipt_not_present(p),
        cose_status_t::COSE_OK
    );
}
...
fn policy_require_receipt_signature_verified() {
    let p = make_policy();
    assert_eq!(
        cose_sign1_mst_trust_policy_builder_require_receipt_signature_verified(p),
        cose_status_t::COSE_OK
    );
}
...
fn policy_require_receipt_signature_not_verified() {
    let p = make_policy();
    assert_eq!(
        cose_sign1_mst_trust_policy_builder_require_receipt_signature_not_verified(p),
        cose_status_t::COSE_OK
    );
}
...
fn policy_require_receipt_issuer_contains() {
    ...
    assert_eq!(
        cose_sign1_mst_trust_policy_builder_require_receipt_issuer_contains(p, needle.as_ptr()),
        cose_status_t::COSE_OK
    );
}
...
fn policy_require_receipt_issuer_eq() {
    ...
    assert_eq!(
        cose_sign1_mst_trust_policy_builder_require_receipt_issuer_eq(p, issuer.as_ptr()),
        cose_status_t::COSE_OK
    );
}
...
fn policy_require_receipt_kid_eq() {
    ...
    assert_eq!(
        cose_sign1_mst_trust_policy_builder_require_receipt_kid_eq(p, kid.as_ptr()),
        cose_status_t::COSE_OK
    );
}
...
fn policy_require_receipt_kid_contains() {
    ...
    assert_eq!(
        cose_sign1_mst_trust_policy_builder_require_receipt_kid_contains(p, needle.as_ptr()),
        cose_status_t::COSE_OK
    );
}
...
fn policy_require_receipt_trusted() {
    let p = make_policy();
    assert_eq!(
        cose_sign1_mst_trust_policy_builder_require_receipt_trusted(p),
        cose_status_t::COSE_OK
    );
}
...
fn policy_require_receipt_not_trusted() {
    let p = make_policy();
    assert_eq!(
        cose_sign1_mst_trust_policy_builder_require_receipt_not_trusted(p),
        cose_status_t::COSE_OK
    );
}
...
fn policy_require_receipt_statement_sha256_eq() {
    ...
    assert_eq!(
        cose_sign1_mst_trust_policy_builder_require_receipt_statement_sha256_eq(p, hex.as_ptr()),
        cose_status_t::COSE_OK
    );
}
...
fn policy_require_receipt_statement_coverage_eq() {
    ...
    assert_eq!(
        cose_sign1_mst_trust_policy_builder_require_receipt_statement_coverage_eq(p, cov.as_ptr()),
        cose_status_t::COSE_OK
    );
}
...
fn policy_null_builder_errors() {
    assert_ne!(
        cose_sign1_mst_trust_policy_builder_require_receipt_present(ptr::null_mut()),
        cose_status_t::COSE_OK
    );
    assert_ne!(
...
fn policy_null_string_errors() {
    let p = make_policy();
    assert_ne!(
        cose_sign1_mst_trust_policy_builder_require_receipt_issuer_contains(p, ptr::null()),
        cose_status_t::COSE_OK
    );
}
