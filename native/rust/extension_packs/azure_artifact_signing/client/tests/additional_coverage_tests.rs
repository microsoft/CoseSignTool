...
use azure_artifact_signing_client::{
    CertificateProfileClientCreateOptions, OperationStatus, SignOptions, SignStatus, API_VERSION,
};
...
fn test_operation_status_to_poller_status() {
    ...
    assert_eq!(
        ...
        azure_core::http::poller::PollerStatus::Failed
    );
    assert_eq!(
...
fn test_build_sign_request_basic_validation() {
    ...
    assert!(request
        .url()
        .to_string()
        .contains("api-version=2022-06-15-preview"));
}
...
fn test_build_requests_basic_validation() {
    ...
    assert!(root_cert_request
        .url()
        .to_string()
        .contains("sign/rootcert"));
    ...
    assert!(cert_chain_request
        .url()
        .to_string()
        .contains("sign/certchain"));
}

#[test]
fn test_parse_response_edge_cases() {
    use azure_artifact_signing_client::client::{
        parse_certificate_response, parse_eku_response, parse_sign_response,
    };
