...
fn test_build_scitt_cwt_claims_single_cert_success() {
    ...
    assert!(
        claims.issued_at.is_some(),
        "Issued at should be current time"
    );
    assert!(
        claims.not_before.is_some(),
        "Not before should be current time"
    );

    // Verify DID:X509 format
    let issuer = claims.issuer.unwrap();
    assert!(
        issuer.starts_with("did:x509:"),
        "Issuer should be DID:X509 format: {}",
        issuer
    );
}
...
fn test_build_scitt_cwt_claims_timing_consistency() {
    ...
    assert_eq!(
        ...
        "issued_at and not_before should be identical"
    );
...
fn test_build_scitt_cwt_claims_custom_issuer_override() {
    ...
    let custom_claims = CwtClaims::new().with_issuer("custom-issuer".to_string());
...
fn test_build_scitt_cwt_claims_custom_subject_override() {
    ...
    let custom_claims = CwtClaims::new().with_subject("custom-subject".to_string());
...
fn test_build_scitt_cwt_claims_custom_audience() {
    ...
    let custom_claims = CwtClaims::new().with_audience("test-audience".to_string());
...
fn test_build_scitt_cwt_claims_custom_expiration() {
    ...
    let custom_claims = CwtClaims::new().with_expiration_time(9999999999);
...
fn test_build_scitt_cwt_claims_custom_not_before() {
    ...
    let custom_claims = CwtClaims::new().with_not_before(1234567890);
...
fn test_build_scitt_cwt_claims_custom_issued_at() {
    ...
    let custom_claims = CwtClaims::new().with_issued_at(9876543210);
