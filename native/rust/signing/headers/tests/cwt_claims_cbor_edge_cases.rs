...
fn test_cbor_decode_complex_custom_claims() {
    ...
    assert_eq!(
        result.custom_claims.get(&1000),
        Some(&CwtClaimValue::Text("valid".to_string()))
    );
    assert_eq!(
        result.custom_claims.get(&1002),
...
fn test_cbor_decode_mixed_custom_claim_types() {
    ...
    assert_eq!(
        result.custom_claims.get(&100),
        Some(&CwtClaimValue::Text("text-value".to_string()))
    );
    assert_eq!(
        result.custom_claims.get(&101),
        Some(&CwtClaimValue::Integer(999))
    );
    assert_eq!(
        result.custom_claims.get(&102),
        Some(&CwtClaimValue::Integer(-123))
    );
    assert_eq!(
        result.custom_claims.get(&103),
        Some(&CwtClaimValue::Bytes(vec![1, 2, 3, 4]))
    );
    assert_eq!(
        result.custom_claims.get(&104),
...
fn test_cbor_decode_duplicate_labels() {
    ...
    assert_eq!(
        result.custom_claims.get(&100),
        Some(&CwtClaimValue::Text("second-value".to_string()))
    );
}
...
fn test_cbor_roundtrip_edge_case_values() {
    ...
    assert_eq!(
        decoded.custom_claims.get(&i64::MIN),
        Some(&CwtClaimValue::Integer(i64::MAX))
    );
    assert_eq!(
        decoded.custom_claims.get(&i64::MAX),
        Some(&CwtClaimValue::Integer(i64::MIN))
    );
    assert_eq!(
        decoded.custom_claims.get(&0),
        Some(&CwtClaimValue::Bytes(vec![0x00, 0xFF, 0x7F, 0x80]))
    );
