...
impl CodeTransparencyError {
    ...
    pub fn from_http_response(http_status: u16, content_type: Option<&str>, body: &[u8]) -> Self {
        let is_cbor = content_type.map(|ct| ct.contains("cbor")).unwrap_or(false);
        ...
        let message = if let Some(ref pd) = problem_details {
            let mut parts = vec![format!(
                "MST service error (HTTP {})",
                pd.status.unwrap_or(http_status as i64)
            )];
    ...
    pub fn from_azure_error(error: azure_core::Error) -> Self {
        ...
        } = error.kind()
        {
            ...
            if let Some(raw) = raw_response {
                let ct = raw
                    .headers()
                    .get_optional_string(&azure_core::http::headers::CONTENT_TYPE);
