...
fn test_decode_all_header_value_types() {
    ...
    assert_eq!(
        ...
        Some(&CoseHeaderValue::Bytes(b"binary_data".to_vec().into()))
    );
    assert_eq!(
    ...
    assert_eq!(
        msg.unprotected_headers().get(&CoseHeaderLabel::Int(15)),
        Some(&CoseHeaderValue::Int(-999))
    );
    assert_eq!(
        msg.unprotected_headers().get(&CoseHeaderLabel::Int(16)),
        Some(&CoseHeaderValue::Bool(false))
    );
    assert_eq!(
        msg.unprotected_headers()
            .get(&CoseHeaderLabel::Text("text_label".to_string())),
        Some(&CoseHeaderValue::Int(555))
    );

    // Check map value
    match msg.unprotected_headers().get(&CoseHeaderLabel::Int(12)) {
    ...
    // Check array value
    match msg.unprotected_headers().get(&CoseHeaderLabel::Int(13)) {
...
fn test_verify_with_embedded_payload() {
    ...
    let result = msg
        .verify(&verifier, Some(b"external"))
...
fn test_verify_detached() {
    ...
    let result = msg
        .verify_detached(&verifier, b"external_payload", Some(b"aad"))
...
fn test_verify_detached_streaming() {
    ...
    let mut reader = SizedReader::new(
        Cursor::new(payload_data.to_vec()),
        payload_data.len() as u64,
    );

    let verifier = MockCryptoVerifier::new(true);
    let result = msg
        .verify_detached_streaming(&verifier, &mut reader, None)
...
fn test_verify_detached_read() {
    ...
    let verifier = MockCryptoVerifier::new(true);
    let result = msg
        .verify_detached_read(&verifier, &mut reader, Some(b"external"))
...
fn test_verify_streaming_payload() {
    ...
    let verifier = MockCryptoVerifier::new(false);
    let result = msg
        .verify_streaming(&verifier, payload, None)
...
fn test_encode_with_embedded_payload() {
    ...
    assert_eq!(reparsed.payload(), Some(b"embedded_payload".as_slice()));
}
...
fn test_unknown_cbor_type_skip() {
    ...
    assert_eq!(
        ...
        Some(&CoseHeaderValue::Int(i64::MAX))
    );
}
...
fn test_uint_header_value_conversion() {
    ...
    assert_eq!(
        ...
        Some(&CoseHeaderValue::Uint((i64::MAX as u64) + 1))
    );
}
