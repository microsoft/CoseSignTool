...
fn test_certificate_source_options_patterns() {
    // Test the options construction pattern used in AzureTrustedSigningCertificateSource
...
fn test_certificate_source_error_conversion_patterns() {
    ...
    let ats_error = format!("ATS certificate fetch failed: {}", test_error);
    
    assert!(ats_error.contains("ATS certificate fetch failed"));
...
// Note: Full testing of AzureTrustedSigningCertificateSource methods like
// fetch_certificate_chain_pkcs7() and sign_digest() would require network calls
// to the Azure Trusted Signing service. The task specifies "Test only PURE LOGIC (no network)",
// so we focus on:
