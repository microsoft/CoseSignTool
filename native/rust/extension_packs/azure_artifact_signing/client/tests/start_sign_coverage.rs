...
use azure_core::{
    ...
    http::{
        headers::Headers, AsyncRawResponse, ClientOptions, HttpClient, Method, Pipeline, Request,
...
impl MockSignClient {
    ...
    fn add_response_sequence(&self, url_pattern: impl Into<String>, responses: Vec<MockResponse>) {
        self.responses
            .lock()
            .unwrap()
...
fn create_mock_client_with_responses(
    ...
    CertificateProfileClient::new_with_pipeline(options, pipeline).expect("Should create client")
}
...
fn test_mock_client_basic() {
    ...
    rt.block_on(async {
        let mock_client = Arc::new(MockSignClient::new());
        let test_response = MockSignClient::json_response(StatusCode::Ok, json!({"test": "value"}));
...
fn test_start_sign_and_poll_to_completion() {
    ...
    run_sign_test(
        ...
        |client| {
            ...
            let result = client
                .sign("PS256", digest, None)
                .expect("Should complete signing");
            ...
            assert_eq!(
                result.signing_certificate,
                Some("Y2VydGlmaWNhdGVkYXRh".to_string())
            );
        },
    );
}
...
fn test_start_sign_immediate_success() {
    ...
    run_sign_test(
        ...
        |client| {
            ...
            assert_eq!(
                result.signing_certificate,
                Some("aW1tZWRpYXRlY2VydA==".to_string())
            );
        },
    );
}
...
fn test_start_sign_error_response() {
    ...
    let error_response =
        MockSignClient::error_response(StatusCode::BadRequest, "Invalid signature algorithm");

    run_sign_test(vec![error_response], vec![], |client| {
...
fn test_start_sign_operation_failed() {
    ...
    run_sign_test(vec![initial_response], vec![failed_response], |client| {
...
fn test_start_sign_timed_out_operation() {
    ...
    run_sign_test(vec![initial_response], vec![timeout_response], |client| {
...
fn test_start_sign_not_found_operation() {
    ...
    run_sign_test(vec![initial_response], vec![not_found_response], |client| {
...
fn test_start_sign_malformed_json_response() {
    ...
    run_sign_test(vec![malformed_response], vec![], |client| {
