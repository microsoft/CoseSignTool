...
/// # Safety
///
/// - `arr` must be null (returns empty vec) or point to a null-terminated array of
///   null-terminated UTF-8 C strings. Each element and the sentinel must be readable.
unsafe fn string_array_to_vec(arr: *const *const c_char) -> Vec<String> {
    ...
    let mut ptr = arr;
    loop {
        // SAFETY: `ptr` is within the null-terminated array; we stop when we hit the null sentinel.
        let s = unsafe { *ptr };
        if s.is_null() {
            break;
        }
        // SAFETY: `s` is a non-null pointer to a null-terminated C string per the function contract.
        if let Ok(cstr) = unsafe { CStr::from_ptr(s).to_str() } {
            result.push(cstr.to_string());
        }
        // SAFETY: Advancing within the null-terminated array; next element is valid or the sentinel.
...
/// # Safety
///
/// - `builder` must be a valid pointer from `cose_sign1_validator_builder_new()`, or null (returns `COSE_ERR`).
#[no_mangle]
...
///
/// - `builder` must be a valid pointer from `cose_sign1_validator_builder_new()`, or null (returns `COSE_ERR`).
/// - `options` may be null (defaults are used). If non-null, it must point to a valid
///   `cose_certificate_trust_options_t` whose `allowed_thumbprints` and `pqc_algorithm_oids`
///   fields are either null or point to null-terminated arrays of null-terminated UTF-8 C strings.
#[no_mangle]
pub extern "C" fn cose_sign1_validator_builder_with_certificates_pack_ex(
    ...
    with_catch_unwind(|| {
        ...
        let opts = if options.is_null() {
            ...
            // SAFETY: `options` is non-null; dereferencing a valid `cose_certificate_trust_options_t`.
            ...
            CertificateTrustOptions {
                ...
                // SAFETY: `allowed_thumbprints` is null or a null-terminated string array per contract.
                allowed_thumbprints: unsafe { string_array_to_vec(opts_ref.allowed_thumbprints) },
                // SAFETY: `pqc_algorithm_oids` is null or a null-terminated string array per contract.
...
/// # Safety
///
/// - `cert_der` must be a valid pointer to `cert_der_len` bytes of DER-encoded certificate data,
///   or null (returns `COSE_ERR`).
/// - `out_key` must be a valid, non-null, aligned pointer. On success the caller owns the returned
///   handle and must free it with `cosesign1_key_free`.
///
/// # Arguments
///
