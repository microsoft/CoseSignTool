...
use cose_sign1_validation::fluent::{
    CoseKeyResolutionResult, CoseKeyResolver, CoseSign1CompiledTrustPlan, CoseSign1Message,
    CoseSign1TrustPack, CoseSign1ValidationError, CoseSign1ValidationOptions, CoseSign1Validator,
    CounterSignatureResolutionResult, CryptoError, CryptoVerifier, Payload,
    PostSignatureValidationContext, PostSignatureValidator, ValidationFailure, ValidationResult,
...
fn test_validation_result_methods() {
    ...
    assert_eq!(
        failure_result.failures[0].error_code.as_deref(),
        Some("TEST_ERROR_CODE")
    );
...
fn test_cose_key_resolution_result_failure() {
    ...
    assert_eq!(
        ...
        Some("Unable to resolve signing key")
    );
}
...
fn test_counter_signature_resolution_result_failure() {
    ...
    assert_eq!(
        result.error_message.as_deref(),
        Some("Counter signature resolution failed")
    );
}
...
impl TrustFactProducer for MockFactProducer {
    ...
    fn produce(&self, _context: &mut TrustFactContext) -> Result<(), TrustError> {
        Ok(())
    }
...
impl CoseKeyResolver for MockKeyResolver {
    fn resolve(
        ...
        if self.should_succeed {
            CoseKeyResolutionResult::success(Arc::new(MockVerifier {
                should_succeed: true,
                should_error: false,
            }))
        } else {
            CoseKeyResolutionResult::failure(
                ...
                Some("Mock resolver failure".to_string()),
            )
        }
    }
    ...
    fn resolve_async<'a>(
        ...
        Box::pin(async move {
            ...
            if self.should_succeed {
                ...
                CoseKeyResolutionResult::failure(
                    Some("ASYNC_TEST_ERROR".to_string()),
                    Some("Mock async resolver failure".to_string()),
                )
            }
        })
    }
...
fn test_validator_with_multiple_trust_packs() {
    ...
    let plan = CoseSign1CompiledTrustPlan::from_parts(allow_all_plan(), packs).unwrap();
...
fn test_validator_async_path_with_failed_resolution() {
    ...
    let plan = CoseSign1CompiledTrustPlan::from_parts(allow_all_plan(), vec![pack]).unwrap();
...
fn test_validator_async_bytes_path() {
    ...
    let result = block_on(validator.validate_bytes_async(EverParseCborProvider, bytes));
...
fn test_validator_with_detached_payload() {
    ...
    let validator = CoseSign1Validator::new(plan).with_options(|opts| {
...
fn test_validator_counter_signature_bypass_logic() {
    ...
    let plan =
        CoseSign1CompiledTrustPlan::from_parts(allow_all_plan(), vec![failing_pack]).unwrap();
...
fn test_validation_result_helper_methods() {
    ...
    let failure_result =
        ValidationResult::failure_message("TestValidator", "Test failure", Some("TEST_CODE"));
    assert!(!failure_result.is_valid());
    assert!(failure_result.is_failure());
...
fn test_cose_key_resolution_result_helpers() {
    let success_result = CoseKeyResolutionResult::success(Arc::new(MockVerifier {
        should_succeed: true,
        should_error: false,
    }));
