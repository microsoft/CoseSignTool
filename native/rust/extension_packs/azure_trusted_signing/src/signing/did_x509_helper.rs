...
//! DID:x509 identifier construction for Azure Trusted Signing certificates.
//!
//! Maps V2 `AzureTrustedSigningDidX509` — generates DID:x509 identifiers
...
/// Microsoft reserved EKU OID prefix used by Azure Trusted Signing certificates.
const MICROSOFT_EKU_PREFIX: &str = "1.3.6.1.4.1.311";

/// Build a DID:x509 identifier from an ATS-issued certificate chain.
///
/// Uses ATS-specific logic:
...
pub fn build_did_x509_from_ats_chain(chain_ders: &[&[u8]]) -> Result<String, AtsError> {
    // Try ATS-specific Microsoft EKU selection first
    if let Some(microsoft_eku) = find_deepest_greatest_microsoft_eku(chain_ders) {
        ...
        did_x509::DidX509Builder::build_from_chain(chain_ders, &[policy])
            .map_err(|e| AtsError::DidX509Error(e.to_string()))
    } else {
...
///
/// Maps V2 `AzureTrustedSigningDidX509.GetDeepestGreatestMicrosoftEku()`.
///
/// Selection criteria:
