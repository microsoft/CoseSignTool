...
use crate::validation::receipt_verify::{
    get_cwt_issuer_host, verify_mst_receipt, ReceiptVerifyInput, CWT_CLAIMS_LABEL, CWT_ISS_LABEL,
};
...
pub fn get_receipts_from_message(msg: &CoseSign1Message) -> Result<Vec<ExtractedReceipt>, String> {
    ...
    for (idx, raw_bytes) in blobs.into_iter().enumerate() {
        ...
        let issuer = match &parsed {
            Ok(m) => get_cwt_issuer_host(m.protected.headers(), CWT_CLAIMS_LABEL, CWT_ISS_LABEL)
...
pub fn get_receipt_issuer_host(receipt_bytes: &[u8]) -> Result<String, String> {
    ...
    get_cwt_issuer_host(receipt.protected.headers(), CWT_CLAIMS_LABEL, CWT_ISS_LABEL)
...
pub fn verify_transparent_statement(
    ...
    let msg =
        CoseSign1Message::parse(bytes).map_err(|e| vec![format!("failed to parse: {}", e)])?;
...
pub fn verify_transparent_statement_message(
    ...
    if receipt_list.is_empty() {
        return Err(vec![
            "No receipts found in the transparent statement.".into()
        ]);
    }
    ...
    for receipt in &receipt_list {
        ...
        match verify_mst_receipt(input) {
            Ok(result) if result.trusted => {
                if is_authorized {
                    valid_authorized.insert(issuer_lower);
                }
                if used_cache {
        ...
        match options.authorized_receipt_behavior {
            AuthorizedReceiptBehavior::VerifyAnyMatching => {
                if valid_authorized.is_empty() {
                    authorized_failures
                        .push("No valid receipts found for any authorized issuer domain.".into());
                } else {
