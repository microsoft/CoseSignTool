...
fn roundtrip_array_value() {
    ...
    match decoded.get(&CoseHeaderLabel::Int(100)).unwrap() {
        CoseHeaderValue::Array(arr) => {
            ...
            assert_eq!(arr[1], CoseHeaderValue::Text("hello".to_string().into()));
            assert_eq!(arr[2], CoseHeaderValue::Bytes(vec![0xAA].into()));
        }
...
fn roundtrip_tagged_value() {
    ...
    map.insert(
        ...
        CoseHeaderValue::Tagged(18, Box::new(CoseHeaderValue::Bytes(vec![0x01].into()))),
    );
    let bytes = map.encode().unwrap();
    let decoded = CoseHeaderMap::decode(&bytes).unwrap();
    match decoded.get(&CoseHeaderLabel::Int(300)).unwrap() {
        CoseHeaderValue::Tagged(tag, inner) => {
            assert_eq!(*tag, 18);
            assert_eq!(inner.as_ref(), &CoseHeaderValue::Bytes(vec![0x01].into()));
        }
...
fn roundtrip_uint_value() {
    ...
    map.insert(CoseHeaderLabel::Int(700), CoseHeaderValue::Uint(u64::MAX));
    let bytes = map.encode().unwrap();
    let decoded = CoseHeaderMap::decode(&bytes).unwrap();
    assert_eq!(
...
fn display_float_raw() {
    ...
    assert_eq!(
        format!("{}", CoseHeaderValue::Raw(vec![0x01, 0x02].into())),
        "raw(2)"
    );
}
...
fn roundtrip_all_value_types() {
    ...
    map.insert(
        ...
        CoseHeaderValue::Map(vec![(CoseHeaderLabel::Int(1), CoseHeaderValue::Int(20))]),
    );
    map.insert(
