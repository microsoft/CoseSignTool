...
use cose_sign1_certificates_local::{CertLocalError, HashAlgorithm, KeyAlgorithm, KeyUsageFlags};
...
fn test_cert_local_error_from_crypto_error() {
    ...
    assert_eq!(
        error.to_string(),
        "key generation failed: test crypto error"
    );
}
...
fn test_key_usage_flags_default() {
    ...
    assert_eq!(
        custom.flags & KeyUsageFlags::DIGITAL_SIGNATURE.flags,
        KeyUsageFlags::DIGITAL_SIGNATURE.flags
    );
    assert_eq!(
        custom.flags & KeyUsageFlags::KEY_CERT_SIGN.flags,
        KeyUsageFlags::KEY_CERT_SIGN.flags
    );
}
    ...
    for algorithm in &algorithms {
        ...
        match algorithm {
            KeyAlgorithm::Rsa => {
                assert!(
                    key_size >= 2048,
                    "RSA key size should be at least 2048 bits"
                );
            }
            KeyAlgorithm::Ecdsa => {
                assert!(
                    key_size == 256 || key_size == 384 || key_size == 521,
