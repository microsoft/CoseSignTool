...
//! Deep coverage tests for azure_trusted_signing_client crate.
//!
//! Targets testable functions that don't require Azure credentials:
//! - AtsClientError Display variants
//! - AtsClientError std::error::Error impl
...
use azure_trusted_signing_client::error::AtsClientError;
use azure_trusted_signing_client::models::*;
use azure_trusted_signing_client::client::*;
use azure_core::http::Url;

// =========================================================================
// AtsClientError Display coverage
// =========================================================================

#[test]
fn error_display_http_error() {
    let e = AtsClientError::HttpError("connection refused".to_string());
    let s = format!("{}", e);
...
fn error_display_authentication_failed() {
    let e = AtsClientError::AuthenticationFailed("token expired".to_string());
    let s = format!("{}", e);
...
fn error_display_service_error_with_target() {
    let e = AtsClientError::ServiceError {
...
fn error_display_operation_failed() {
    let e = AtsClientError::OperationFailed {
...
fn error_display_operation_timeout() {
    let e = AtsClientError::OperationTimeout {
...
fn error_display_deserialization_error() {
    let e = AtsClientError::DeserializationError("invalid json".to_string());
    let s = format!("{}", e);
...
fn error_display_invalid_configuration() {
    let e = AtsClientError::InvalidConfiguration("missing endpoint".to_string());
    let s = format!("{}", e);
...
fn error_display_certificate_chain_not_available() {
    let e = AtsClientError::CertificateChainNotAvailable("404".to_string());
    let s = format!("{}", e);
...
fn error_display_sign_failed() {
    let e = AtsClientError::SignFailed("HSM error".to_string());
    let s = format!("{}", e);
...
#[test]
fn error_debug() {
    let e = AtsClientError::SignFailed("debug test".to_string());
    let debug = format!("{:?}", e);
