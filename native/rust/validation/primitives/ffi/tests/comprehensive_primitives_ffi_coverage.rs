...
fn test_trust_plan_builder_new_null_validator() {
    let mut plan_builder: *mut cose_sign1_trust_plan_builder_t = ptr::null_mut();
    let result = unsafe {
        cose_sign1_trust_plan_builder_new_from_validator_builder(ptr::null(), &mut plan_builder)
    };
...
fn test_trust_plan_builder_new_null_out_ptr() {
    ...
    let result = unsafe {
        cose_sign1_trust_plan_builder_new_from_validator_builder(validator_builder, ptr::null_mut())
    };
...
fn test_trust_plan_builder_compile_deny_all() {
    ...
    let mut compiled_plan: *mut cose_sign1_compiled_trust_plan_t = ptr::null_mut();
    let result =
        unsafe { cose_sign1_trust_plan_builder_compile_deny_all(plan_builder, &mut compiled_plan) };
    assert_eq!(result, cose_status_t::COSE_OK);
    assert!(!compiled_plan.is_null());
...
fn test_validator_builder_with_compiled_trust_plan() {
    ...
    let result = unsafe {
        cose_sign1_validator_builder_with_compiled_trust_plan(validator_builder, compiled_plan)
    };
    assert_eq!(result, cose_status_t::COSE_OK);
...
fn test_trust_policy_builder_cwt_time_requirements() {
    ...
    let result =
        unsafe { cose_sign1_trust_policy_builder_require_cwt_exp_ge(policy_builder, test_time) };
    assert_eq!(result, cose_status_t::COSE_OK);

    let result =
        unsafe { cose_sign1_trust_policy_builder_require_cwt_exp_le(policy_builder, test_time) };
    assert_eq!(result, cose_status_t::COSE_OK);

    // Test not before requirements
    let result =
        unsafe { cose_sign1_trust_policy_builder_require_cwt_nbf_ge(policy_builder, test_time) };
    assert_eq!(result, cose_status_t::COSE_OK);

    let result =
        unsafe { cose_sign1_trust_policy_builder_require_cwt_nbf_le(policy_builder, test_time) };
    assert_eq!(result, cose_status_t::COSE_OK);

    // Test issued at requirements
    let result =
        unsafe { cose_sign1_trust_policy_builder_require_cwt_iat_ge(policy_builder, test_time) };
    assert_eq!(result, cose_status_t::COSE_OK);

    let result =
        unsafe { cose_sign1_trust_policy_builder_require_cwt_iat_le(policy_builder, test_time) };
    assert_eq!(result, cose_status_t::COSE_OK);
...
fn test_trust_policy_builder_cwt_claim_label_requirements() {
    ...
    let result = unsafe {
        cose_sign1_trust_policy_builder_require_cwt_claim_label_i64_eq(policy_builder, 100, 42)
    };
    assert_eq!(result, cose_status_t::COSE_OK);

    let result = unsafe {
        cose_sign1_trust_policy_builder_require_cwt_claim_label_i64_ge(policy_builder, 101, 10)
    };
    assert_eq!(result, cose_status_t::COSE_OK);

    let result = unsafe {
        cose_sign1_trust_policy_builder_require_cwt_claim_label_i64_le(policy_builder, 102, 100)
    };
    assert_eq!(result, cose_status_t::COSE_OK);

    // Test boolean comparison
    let result = unsafe {
        cose_sign1_trust_policy_builder_require_cwt_claim_label_bool_eq(policy_builder, 103, true)
    };
    assert_eq!(result, cose_status_t::COSE_OK);
...
fn test_trust_policy_builder_null_pointer_safety() {
    ...
    assert_ne!(
        unsafe { cose_sign1_trust_policy_builder_and(ptr::null_mut()) },
        cose_status_t::COSE_OK
    );
    assert_ne!(
        unsafe { cose_sign1_trust_policy_builder_or(ptr::null_mut()) },
        cose_status_t::COSE_OK
    );
    assert_ne!(
...
fn test_trust_plan_builder_null_pointer_safety() {
    ...
    assert_ne!(
        unsafe {
            cose_sign1_trust_plan_builder_compile_allow_all(ptr::null_mut(), &mut compiled_plan)
        },
        cose_status_t::COSE_OK
    );
    assert_ne!(
        unsafe {
            cose_sign1_trust_plan_builder_compile_deny_all(ptr::null_mut(), &mut compiled_plan)
        },
        cose_status_t::COSE_OK
    );
}
