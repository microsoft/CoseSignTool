...
/// Mock crypto signer for building test messages without real keys.
...
fn create_test_message() -> CoseSign1Message {
    ...
    CoseSign1Message::parse(&bytes).expect("Failed to parse test message")
}
...
fn test_add_proof_with_receipt_merge_success() {
    ...
    assert_eq!(receipts.len(), 1);
    assert_eq!(receipts[0].as_ref(), b"receipt-test");
}
...
fn test_add_proof_with_receipt_merge_preserve_existing() {
    ...
    let receipts = extract_receipts(&result_msg);
    assert_eq!(receipts.len(), 2);
    assert!(receipts.iter().any(|r| r.as_ref() == b"existing-receipt"));
    assert!(receipts.iter().any(|r| r.as_ref() == b"receipt-test"));
}
