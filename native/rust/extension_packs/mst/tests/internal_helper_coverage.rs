...
use cose_sign1_transparent_mst::validation::receipt_verify::{
    ccf_accumulator_sha256, extract_proof_blobs, get_cwt_issuer_host, is_cose_sign1_tagged_18,
    parse_leaf, parse_path, reencode_statement_with_cleared_unprotected_headers,
    validate_cose_alg_supported, MstCcfInclusionProof, ReceiptVerifyError,
};
    ...
    let pairs = vec![
        (
            ...
            CoseHeaderValue::Array(vec![
                ...
                CoseHeaderValue::Bytes(vec![0x04, 0x05, 0x06].into()),
            ]),
        ),
...
fn test_extract_proof_blobs_missing_proof_label() {
    ...
    let pairs = vec![(
        CoseHeaderLabel::Int(1),
        CoseHeaderValue::Array(vec![CoseHeaderValue::Bytes(vec![0x01, 0x02, 0x03].into())]),
    )];
    let vdp_value = CoseHeaderValue::Map(pairs);
