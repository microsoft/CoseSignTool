...
public:
     ...
     * 
     * @param cose_bytes Pointer to COSE Sign1 message bytes
     * @param cose_len Length of message bytes
     * @param detached_payload Optional detached payload pointer (nullptr for embedded)
     * @param detached_len Length of detached payload (0 if nullptr)
     * @return ValidationResult object
     */
    ValidationResult Validate(
        const uint8_t* cose_bytes,
        size_t cose_len,
        const uint8_t* detached_payload = nullptr,
    ...
    /**
     * @brief Validate COSE Sign1 message bytes (vector overload)
     * 
    ...
    ValidationResult Validate(
        ...
        return Validate(
            cose_bytes.data(),
            cose_bytes.size(),
            detached_payload.empty() ? nullptr : detached_payload.data(),
    ...
    /**
     * @brief Validate a CoseSign1Message directly (zero-copy)
     *
     * Borrows the message's backing bytes — no copy. This is the
     * preferred path when you already have a CoseSign1Message from
     * signing or parsing.
     * 
     * @param message The parsed/signed message
     * @param detached_payload Optional detached payload pointer (nullptr for embedded)
     * @param detached_len Length of detached payload
     * @return ValidationResult object
     */
    ValidationResult Validate(
        const CoseSign1Message& message,
        const uint8_t* detached_payload = nullptr,
        size_t detached_len = 0
    ) const {
        auto bytes = message.AsBytes();
        return Validate(bytes.data, bytes.size, detached_payload, detached_len);
    }
#endif
