...
use cose_sign1_azure_trusted_signing::validation::{AtsFactProducer, AzureTrustedSigningTrustPack};
use cose_sign1_azure_trusted_signing::validation::facts::AtsSigningServiceIdentifiedFact;
...
fn test_ats_fact_producer_produce() {
    ...
    let result = engine.get_facts::<AtsSigningServiceIdentifiedFact>(&subject);
...
fn test_azure_trusted_signing_trust_pack_new() {
    ...
    assert_eq!(resolvers.len(), 0); // ATS delegates to certificates pack
...
fn test_trust_pack_fact_producer_consistency() {
    ...
    let standalone_producer = AtsFactProducer;
    
    // Both should have the same name
    assert_eq!(fact_producer_from_pack.name(), standalone_producer.name());
    assert_eq!(fact_producer_from_pack.name(), "azure_trusted_signing");
}