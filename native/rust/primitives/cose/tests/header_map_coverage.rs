...
fn test_header_value_from_conversions() {
    ...
    assert_eq!(val3, CoseHeaderValue::Bytes(vec![1, 2, 3].into()));

    let val4: CoseHeaderValue = ([1u8, 2, 3].as_slice()).into();
    assert_eq!(val4, CoseHeaderValue::Bytes(vec![1, 2, 3].into()));

    let val5: CoseHeaderValue = "test".to_string().into();
    assert_eq!(val5, CoseHeaderValue::Text("test".to_string().into()));

    let val6: CoseHeaderValue = "test".into();
    assert_eq!(val6, CoseHeaderValue::Text("test".to_string().into()));
...
fn test_header_value_accessors() {
    // Test as_bytes
    let bytes_val = CoseHeaderValue::Bytes(vec![1, 2, 3].into());
    assert_eq!(bytes_val.as_bytes(), Some(&[1u8, 2, 3][..]));

    let text_val = CoseHeaderValue::Text("test".to_string().into());
    ...
    assert_eq!(
        array_val.as_bytes_one_or_many(),
...
fn test_header_map_crit_edge_cases() {
    ...
    let crit_array = vec![
        ...
        CoseHeaderValue::Bytes(vec![1, 2, 3].into()), // Invalid - should be filtered out
    ];
    map.insert(
...
fn test_header_map_get_bytes_one_or_many() {
    ...
    assert_eq!(
        map.get_bytes_one_or_many(&CoseHeaderLabel::Int(33)),
        Some(vec![vec![1, 2, 3]])
    );
    ...
    assert_eq!(
        map.get_bytes_one_or_many(&CoseHeaderLabel::Int(34)),
        Some(vec![vec![1, 2], vec![3, 4]])
    );
...
fn test_header_map_encode_decode_roundtrip() {
    ...
    map.insert(CoseHeaderLabel::Int(1), CoseHeaderValue::Int(-7));
    map.insert(
        CoseHeaderLabel::Text("test".to_string()),
        CoseHeaderValue::Text("value".to_string().into()),
    );
    map.insert(
    ...
    assert_eq!(
        decoded.get(&CoseHeaderLabel::Text("test".to_string())),
...
fn test_header_map_encode_complex_structures() {
    ...
    map.insert(
        ...
        CoseHeaderValue::Raw(vec![0xf6].into()),
    ); // null in CBOR
    ...
    match decoded.get(&CoseHeaderLabel::Int(102)) {
        Some(CoseHeaderValue::Tagged(tag, inner)) => {
            assert_eq!(*tag, 42);
            assert_eq!(**inner, CoseHeaderValue::Text("tagged".to_string().into()));
        }
...
fn test_header_map_decode_indefinite_map() {
    ...
    encoder
        .encode_map_indefinite_begin()
        .expect("encode indefinite map");
...
fn test_decode_invalid_header_label() {
    ...
    encoder.encode_map(1).expect("encode map");
    encoder
        .encode_bstr(b"invalid")
        .expect("encode invalid label");
    ...
    assert!(result
        .unwrap_err()
        .to_string()
        .contains("invalid header label type"));
}
...
fn test_protected_header_get() {
    ...
    assert_eq!(
        protected.get(&CoseHeaderLabel::Int(999)),
