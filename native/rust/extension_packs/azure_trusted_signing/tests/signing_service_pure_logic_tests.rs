...
fn test_ats_signing_key_provider_adapter_is_remote() {
    // Test the SigningKeyProvider is_remote() method always returns true for ATS
    // This is a structural property of ATS — it's always a remote HSM
...
fn test_ats_certificate_source_adapter_has_private_key() {
    // Test that has_private_key() always returns false for ATS
...
fn test_ats_crypto_signer_construction() {
    // Test AtsCryptoSigner construction with various algorithms
    let algorithms = vec![
        ("RS256", -257, "RSA"),
        ("RS384", -258, "RSA"),
        ("RS512", -259, "RSA"),
...
fn test_ats_scitt_compliance_enabled() {
    // Test that SCITT compliance is always enabled for ATS
...
fn test_ats_error_conversion_to_signing_error() {
    // Test error conversion patterns from ATS errors to SigningError
    let ats_error_msg = "Failed to fetch certificate from ATS";
    let signing_error = format!("KeyError: {}", ats_error_msg);
    
    assert!(signing_error.contains("KeyError"));
    assert!(signing_error.contains("Failed to fetch certificate from ATS"));
}
...
fn test_ats_certificate_signing_options_pattern() {
    // Test CertificateSigningOptions construction with ATS-specific settings
...
fn test_ats_service_delegation_pattern() {
    // Test that AzureTrustedSigningService delegates to CertificateSigningService
    // This tests the composition pattern over inheritance
    
    let is_remote = true; // ATS is always remote
    
    // Verify delegation pattern: ATS.is_remote() -> inner.is_remote() -> true
    assert!(is_remote);
}

#[test]
fn test_ats_primary_algorithm() {
    // Test that ATS primarily uses PS256 (RSA-PSS)
...
fn test_ats_build_did_issuer_error_message_format() {
    ...
    let ats_did_error = "missing required EKU";
    let signing_error = format!("ATS DID:x509 generation failed: {}", ats_did_error);
    
    assert!(signing_error.contains("ATS DID:x509 generation failed"));
...
fn test_ats_root_cert_fetch_error_format() {
    ...
    let signing_error = format!("Failed to fetch ATS root cert for DID:x509: {}", fetch_error);
    
    assert!(signing_error.contains("Failed to fetch ATS root cert for DID:x509"));
