...
public abstract class TransparencyProviderBase : ITransparencyProvider
{
    ...
    internal static class ClassStrings
    {
        ...
        public const string ErrorReceiptEmpty = "The receipt cannot be empty.";
    ...
    /// <summary>
    /// Verifies the transparency proof in a message using a specific receipt.
    /// The receipt is embedded into the message before verification.
    /// </summary>
    /// <param name="message">The COSE Sign1 message to verify.</param>
    /// <param name="receipt">The receipt to embed and verify against.</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>Validation result with status and details.</returns>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="message"/> or <paramref name="receipt"/> is <see langword="null"/>.</exception>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (receipt.Length == 0)
        {
            throw new ArgumentOutOfRangeException(nameof(receipt), ClassStrings.ErrorReceiptEmpty);
        }

        // Embed the receipt into the message's unprotected headers for verification
        MergeReceipts(message, new List<byte[]> { receipt });

        // Verify the message with the embedded receipt
        return VerifyTransparencyProofAsync(message, cancellationToken);
    }
