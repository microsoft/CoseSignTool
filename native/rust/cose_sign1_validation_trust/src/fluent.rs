...
use crate::rules::{all_of, any_of, require_fact_matches, FactSelector, OnEmptyBehavior, PropertyPredicate};
...
) -> impl Fn(&TrustSubject) -> TrustSubject + Send + Sync + 'static {
    move |s: &TrustSubject| match target {
        SubjectTarget::Message => s.clone(),
        SubjectTarget::PrimarySigningKey => TrustSubject::primary_signing_key(s),
    }
}

/// Fluent rule builder for common trust policies.
///
/// Goal: policies read naturally and only reference fact *types* and typed *fields*.
/// No string field names and no custom rule closures.
...
impl Rules {
    ...
    /// Scope subsequent requirements to the message subject.
    pub fn for_message(self) -> TargetRules {
        TargetRules {
            parent: self,
            target: SubjectTarget::Message,
        }
    }

    /// Scope subsequent requirements to the primary signing key subject.
    pub fn for_primary_signing_key(self) -> TargetRules {
        TargetRules {
            parent: self,
            target: SubjectTarget::PrimarySigningKey,
        }
    }

    /// Apply a scoped policy builder to the primary signing key.
    pub fn with_primary_signing_key(self, f: impl FnOnce(TargetRules) -> TargetRules) -> Self {
        f(self.for_primary_signing_key()).finish()
    }

    pub fn require<TFact>(self) -> Require<TFact> {
        Require {
            parent: self,
            target: SubjectTarget::Message,
            selector: FactSelector::first(),
            deny_reason: "RequirementNotSatisfied",
            name: std::any::type_name::<TFact>(),
            _phantom: PhantomData,
        }
    }

    pub fn and(mut self, rule: TrustRuleRef) -> Self {
        self.rules.push(rule);
        self
    }

    pub fn compile(self) -> CompiledTrustPlan {
        let root = all_of("root", self.rules);
        CompiledTrustPlan::new(self.required_facts, Vec::new(), vec![root], Vec::new())
    }
}
...
fn compile_dnf(name: &'static str, dnf: Vec<Vec<TrustRuleRef>>) -> TrustRuleRef {
    ...
    for conj in dnf.into_iter() {
        and_terms.push(all_of(name, conj));
    }
...
/// A `Rules` builder scoped to a specific subject target.
#[derive(Clone)]
...
impl TargetRules {
    pub fn require<TFact>(self) -> TargetRequire<TFact> {
        TargetRequire {
            parent: self,
            selector: FactSelector::first(),
            deny_reason: "RequirementNotSatisfied",
    ...
    pub fn and(self, rule: TrustRuleRef) -> Self {
        Self {
            parent: self.parent.and(rule),
            target: self.target,
        }
    }

    pub fn finish(self) -> Rules {
        self.parent
    }
...
/// In-progress `Require<TFact>(...)` clause.
#[derive(Clone)]
pub struct Require<TFact> {
    parent: Rules,
    target: SubjectTarget,
    selector: FactSelector,
    name: &'static str,
...
impl<TFact> Require<TFact>
where
    TFact: crate::fact_properties::FactProperties + Send + Sync + 'static,
{
    pub fn on_message(mut self) -> Self {
        self.target = SubjectTarget::Message;
        self
    }

    pub fn on_primary_signing_key(mut self) -> Self {
        self.target = SubjectTarget::PrimarySigningKey;
        self
    }
    ...
    pub fn deny_reason(mut self, reason: &'static str) -> Self {
        self.deny_reason = reason;
        self
    }

    /// Configure this requirement's predicates via a closure (lambda-like) builder.
    pub fn where_(mut self, f: impl FnOnce(Where<TFact>) -> Where<TFact>) -> Self {
        let where_builder = Where {
            selector: self.selector,
            _phantom: PhantomData,
        };
        self.selector = f(where_builder).selector;
        self
    }

    /// Boolean field must be `true`.
    pub fn where_true(mut self, field: Field<TFact, bool>) -> Self {
        self.selector = self.selector.where_pred(
            field.name(),
            PropertyPredicate::Eq(crate::fact_properties::FactValueOwned::Bool(true)),
        );
    ...
    /// Boolean field must be `false`.
    pub fn where_false(mut self, field: Field<TFact, bool>) -> Self {
        self.selector = self.selector.where_pred(
            field.name(),
            PropertyPredicate::Eq(crate::fact_properties::FactValueOwned::Bool(false)),
        );
    ...
    pub fn where_usize_eq(mut self, field: Field<TFact, usize>, expected: usize) -> Self {
        self.selector = self.selector.where_pred(
            field.name(),
            PropertyPredicate::Eq(crate::fact_properties::FactValueOwned::Usize(expected)),
        );
    ...
    pub fn where_u32_eq(mut self, field: Field<TFact, u32>, expected: u32) -> Self {
        self.selector = self.selector.where_pred(
            field.name(),
            PropertyPredicate::Eq(crate::fact_properties::FactValueOwned::U32(expected)),
        );
    ...
    pub fn where_i64_ge(mut self, field: Field<TFact, i64>, min: i64) -> Self {
        self.selector = self
            .selector
            .where_pred(field.name(), PropertyPredicate::NumGeI64(min));
        self
    ...
    pub fn where_str_non_empty(mut self, field: Field<TFact, String>) -> Self {
        self.selector = self
            .selector
            .where_pred(field.name(), PropertyPredicate::StrNonEmpty);
        self
    ...
    pub fn where_str_matches_regex(
        mut self,
        field: Field<TFact, String>,
        pattern: impl Into<String>,
    ) -> Self {
    ...
    /// Finish this requirement and start another requirement (AND semantics).
    pub fn and(self) -> Rules {
        let subject_selector = selector_for_target(self.target);
        let rule = require_fact_matches::<TFact, _>(
            self.name,
            subject_selector,
            self.selector,
            self.deny_reason,
        );
        self.parent.and(rule)
    }

    /// Finish and compile a plan.
    pub fn compile(self) -> CompiledTrustPlan {
        self.and().compile()
    }
}

/// In-progress `Require<TFact>(...)` clause with an implicit subject target.
#[derive(Clone)]
pub struct TargetRequire<TFact> {
    parent: TargetRules,
    selector: FactSelector,
    name: &'static str,
    deny_reason: &'static str,
...
impl<TFact> TargetRequire<TFact>
where
    TFact: crate::fact_properties::FactProperties + Send + Sync + 'static,
{
    pub fn named(mut self, name: &'static str) -> Self {
    ...
    /// Finish this requirement and start another requirement (AND semantics).
    pub fn and(self) -> TargetRules {
        let subject_selector = selector_for_target(self.parent.target);
        let rule = require_fact_matches::<TFact, _>(
            self.name,
            subject_selector,
            self.selector,
