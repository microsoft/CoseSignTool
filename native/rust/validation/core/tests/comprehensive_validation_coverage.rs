...
fn test_validator_comprehensive_scenarios() {
    ...
    let _result1 = validator.validate_bytes(
        EverParseCborProvider,
        Arc::from(large_cose.into_boxed_slice()),
    );
    ...
    enc.encode_bstr(b"complex test payload with json data")
        .unwrap();
    enc.encode_bstr(&[0u8; 64]).unwrap();
    let complex_cose = enc.into_bytes();

    let _result2 = validator.validate_bytes(
        EverParseCborProvider,
        Arc::from(complex_cose.into_boxed_slice()),
    );
    ...
    let _result3 = validator.validate_bytes(
        EverParseCborProvider,
        Arc::from(detached_cose.into_boxed_slice()),
    );
}
...
fn test_message_facts_and_indirect_signatures() {
    ...
    let _result = validator.validate_bytes(
        EverParseCborProvider,
        Arc::from(complex_message.into_boxed_slice()),
    );
    ...
    let _error_result = validator.validate_bytes(
        EverParseCborProvider,
    ...
    let _malformed_result = validator.validate_bytes(
        EverParseCborProvider,
        Arc::from(malformed_cose.into_boxed_slice()),
    );
}
...
fn test_trust_plan_builder_comprehensive() {
    ...
    let minimal_plan = empty_builder
        .for_message(|msg| msg.require_content_type_non_empty())
        .compile();
...
fn test_cbor_provider_comprehensive() {
    ...
    encoder
        .encode_tstr("test string with unicode: 🔒🛡️")
        .unwrap();
    encoder
        .encode_bstr(&[0x01, 0x02, 0x03, 0x04, 0x05])
        .unwrap();
...
fn test_parsed_message_comprehensive() {
    ...
    if let Ok(message) = CoseSign1Message::parse(&cose_bytes) {
        ...
        let pack =
            Arc::new(SimpleTrustPack::no_facts("parsed-test")) as Arc<dyn CoseSign1TrustPack>;
        let validator = CoseSign1Validator::new(vec![pack]);

        let _parsed_result = validator.validate_bytes(
            EverParseCborProvider,
            Arc::from(cose_bytes.into_boxed_slice()),
        );
    }
