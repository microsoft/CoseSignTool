...
pub struct AzureTrustedSigningCertificateSource {
    client: CertificateProfileClient,
}

impl AzureTrustedSigningCertificateSource {
    /// Create with DefaultAzureCredential (for local dev / managed identity).
    #[cfg_attr(coverage_nightly, coverage(off))]
    pub fn new(options: AzureTrustedSigningOptions) -> Result<Self, AtsError> {
        ...
        let client = CertificateProfileClient::new_dev(client_options)
            .map_err(|e| AtsError::CertificateFetchFailed(e.to_string()))?;
        Ok(Self { client })
    }
    ...
    #[cfg_attr(coverage_nightly, coverage(off))]
    pub fn fetch_certificate_chain_pkcs7(&self) -> Result<Vec<u8>, AtsError> {
        self.client
    ...
    #[cfg_attr(coverage_nightly, coverage(off))]
    pub fn fetch_root_certificate(&self) -> Result<Vec<u8>, AtsError> {
        self.client
    ...
    #[cfg_attr(coverage_nightly, coverage(off))]
    pub fn fetch_eku(&self) -> Result<Vec<String>, AtsError> {
        self.client
            .get_eku()
            .map_err(|e| AtsError::CertificateFetchFailed(e.to_string()))
    }

    /// Sign a digest using the ATS HSM (sync — blocks on the Poller internally).
    ///
    ...
    pub fn sign_digest(
        ...
        algorithm: &str,
        digest: &[u8],
    ) -> Result<(Vec<u8>, Vec<u8>), AtsError> {
        let status = self.client
            .sign(algorithm, digest)
            .map_err(|e| AtsError::SigningFailed(e.to_string()))?;
    ...
    pub fn start_sign(
        ...
        algorithm: &str,
        digest: &[u8],
    ) -> Result<azure_core::http::poller::Poller<SignStatus>, AtsError> {
        self.client
    ...
    fn decode_sign_status(status: SignStatus) -> Result<(Vec<u8>, Vec<u8>), AtsError> {
        ...
        let cert_b64 = status.signing_certificate
            .ok_or_else(|| AtsError::SigningFailed("No signing certificate in response".into()))?;

        use base64::Engine;
        let signature = base64::engine::general_purpose::STANDARD.decode(&sig_b64)
            .map_err(|e| AtsError::SigningFailed(format!("Invalid base64 signature: {}", e)))?;
        let cert_der = base64::engine::general_purpose::STANDARD.decode(&cert_b64)
            .map_err(|e| AtsError::SigningFailed(format!("Invalid base64 certificate: {}", e)))?;
