...
use cose_sign1_validation_primitives::{
    facts::TrustFactEngine, plan::CompiledTrustPlan, policy::TrustPolicyBuilder,
...
fn test_async_validate_basic() {
    let packs: Vec<Arc<dyn CoseSign1TrustPack>> = vec![Arc::new(
        SimpleTrustPack::no_facts("test")
            ...
            .with_cose_key_resolver(Arc::new(MockKeyResolver::new(Arc::new(MockVerifier::new(
                -7,
            ))))),
    )];
...
fn test_counter_signature_bypass_metadata() {
    ...
    let packs: Vec<Arc<dyn CoseSign1TrustPack>> = vec![
        Arc::new(SimpleTrustPack::no_facts("test").with_default_trust_plan(allow_all_trust_plan())), // No counter-signature resolver - this should trigger bypass paths
    ];
...
fn test_message_fact_producer_error_branches() {
    ...
    let engine =
        TrustFactEngine::new(vec![Arc::new(producer)]).with_cose_sign1_bytes(invalid_bytes);
