...
fn kid_allowed_fact_properties() {
    ...
    assert_eq!(fact.get_property("is_allowed"), Some(FactValue::Bool(true)));
    assert!(fact.get_property("nonexistent").is_none());
}
...
fn trust_pack_with_patterns() {
    let options = AzureKeyVaultTrustOptions {
        allowed_kid_patterns: vec!["https://myvault.vault.azure.net/keys/*".to_string()],
        ..Default::default()
    };
...
fn fluent_require_kid_not_allowed() {
    ...
    let plan = TrustPlanBuilder::new(vec![pack])
        .for_message(|m| m.require::<AzureKeyVaultKidAllowedFact>(|w| w.require_kid_not_allowed()))
        .compile();
    assert!(plan.is_ok());
}
