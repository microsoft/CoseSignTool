...
fn test_contribute_protected_headers() {
    ...
    assert!(headers
        .get(&CoseHeaderLabel::Int(
            CertificateHeaderContributor::X5T_LABEL
        ))
        .is_some());
    assert!(headers
        .get(&CoseHeaderLabel::Int(
            CertificateHeaderContributor::X5CHAIN_LABEL
        ))
        .is_some());

    // Verify the headers contain raw CBOR data
    let x5t_value = headers
        .get(&CoseHeaderLabel::Int(
            CertificateHeaderContributor::X5T_LABEL,
        ))
        .unwrap();
    let x5chain_value = headers
        .get(&CoseHeaderLabel::Int(
            CertificateHeaderContributor::X5CHAIN_LABEL,
        ))
        .unwrap();
...
fn test_build_x5chain_cbor_array() {
    ...
    if let CoseHeaderValue::Raw(x5chain_bytes) = x5chain_value {
        ...
        for (i, expected_cert) in chain.iter().enumerate() {
            let cert_bytes = decoder
                .decode_bstr()
                .expect(&format!("Should be cert {} bytes", i));
...
fn test_x5t_different_certs_different_thumbprints() {
    ...
    let x5t_value1 = headers1
        .get(&CoseHeaderLabel::Int(
            CertificateHeaderContributor::X5T_LABEL,
        ))
        .unwrap();
    let x5t_value2 = headers2
        .get(&CoseHeaderLabel::Int(
            CertificateHeaderContributor::X5CHAIN_LABEL,
        ))
        .unwrap();
