...
fn test_parse_indefinite_array() {
    ...
    assert!(result
        .unwrap_err()
        .to_string()
        .contains("definite-length array"));
}
...
fn test_decode_header_value_types() {
    ...
    assert_eq!(
        msg.unprotected_headers().get(&CoseHeaderLabel::Int(101)),
        Some(&CoseHeaderValue::Text("test".to_string().into()))
    );
    assert_eq!(
        msg.unprotected_headers().get(&CoseHeaderLabel::Int(102)),
        Some(&CoseHeaderValue::Bool(true))
    );
    assert_eq!(
        msg.unprotected_headers().get(&CoseHeaderLabel::Int(103)),
        Some(&CoseHeaderValue::Undefined)
    );

    match msg.unprotected_headers().get(&CoseHeaderLabel::Int(105)) {
    ...
    match msg.unprotected_headers().get(&CoseHeaderLabel::Int(106)) {
...
fn test_decode_nested_indefinite() {
    ...
    match msg.unprotected_headers().get(&CoseHeaderLabel::Int(200)) {
...
fn test_sig_structure_bytes() {
    ...
    let sig_struct = msg
        .sig_structure_bytes(payload, external_aad)
