...
use crate::models::{JsonWebKey, JwksDocument};
...
use azure_core::http::{
    poller::{
        Poller, PollerContinuation, PollerOptions, PollerResult, PollerState, PollerStatus,
        StatusMonitor,
    },
    Body, ClientOptions, Context, Method, Pipeline, RawResponse, Request, Response,
};
...
impl CodeTransparencyClient {
    ...
    pub fn with_options(
        ...
        Self {
            ...
            config,
            pipeline,
            polling_options: options.polling_options,
            runtime,
        }
    }
    ...
    pub fn with_pipeline(
        ...
        Self {
            ...
            config,
            pipeline,
            polling_options: MstPollingOptions::default(),
            runtime,
        }
    }
    ...
    pub fn get_transparency_config_cbor(&self) -> Result<Vec<u8>, CodeTransparencyError> {
        self.send_get(
            &self.build_url("/.well-known/transparency-configuration"),
            "application/cbor",
        )
    }
    ...
    pub fn create_entry(
        &self,
        cose_bytes: &[u8],
    ) -> Result<Poller<OperationStatus>, CodeTransparencyError> {
        ...
        Ok(Poller::new(
            move |poller_state: PollerState, poller_options| {
                ...
                Box::pin(async move {
                    ...
                    let rsp = pipeline
                        .send(&poller_options.context, &mut request, None)
                        .await?;
                    ...
                    let operation_id =
                        read_cbor_text_field(&body_bytes, "OperationId").unwrap_or_default();
                    ...
                    let monitor_json = serde_json::to_vec(&monitor).map_err(|e| {
                    ...
                    match monitor.status() {
                        PollerStatus::Succeeded => {
                            ...
                            let target_json = serde_json::to_vec(&monitor).map_err(|e| {
                            ...
                            Ok(PollerResult::Succeeded {
                                ...
                                target: Box::new(move || {
                                    Box::pin(async move {
                                        let r: Response<OperationStatus> = RawResponse::from_bytes(
                                            status_code,
                                            headers,
                                            target_json,
                                        )
                        ...
                        _ => {
                            ...
                            let poll_delay =
                                polling_opts.delay_for_retry(attempt, DEFAULT_POLL_INTERVAL);
                            let retry_after =
                                time::Duration::milliseconds(poll_delay.as_millis() as i64);
    ...
    pub fn get_operation(&self, operation_id: &str) -> Result<Vec<u8>, CodeTransparencyError> {
        self.send_get(
            &self.build_url(&format!("/operations/{}", operation_id)),
            "application/cbor",
        )
    }
    ...
    pub fn get_entry(&self, entry_id: &str) -> Result<Vec<u8>, CodeTransparencyError> {
        self.send_get(
            &self.build_url(&format!("/entries/{}", entry_id)),
            "application/cose",
        )
    }
    ...
    pub fn get_entry_statement(&self, entry_id: &str) -> Result<Vec<u8>, CodeTransparencyError> {
        self.send_get(
            &self.build_url(&format!("/entries/{}/statement", entry_id)),
            "application/cose",
        )
    }
    ...
    pub fn resolve_signing_key(&self, kid: &str) -> Result<JsonWebKey, CodeTransparencyError> {
        ...
        if self.config.offline_keys_behavior == OfflineKeysBehavior::OfflineOnly {
            return Err(CodeTransparencyError::HttpError(format!(
                "key '{}' not found in offline keys and network fallback is disabled",
                kid
            )));
        }
