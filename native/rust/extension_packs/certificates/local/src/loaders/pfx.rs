...
pub trait Pkcs12Parser: Send + Sync {
    ...
    fn parse_pkcs12(&self, bytes: &[u8], password: &str) -> Result<ParsedPkcs12, CertLocalError>;
}
...
/// Requires the `pfx` feature.
#[cfg(feature = "pfx")]
pub fn load_from_pfx_no_password<P: AsRef<Path>>(path: P) -> Result<Certificate, CertLocalError> {
    let parser = openssl_impl::OpenSslPkcs12Parser;
