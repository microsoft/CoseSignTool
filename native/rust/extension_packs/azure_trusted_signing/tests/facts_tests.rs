...
use cose_sign1_azure_trusted_signing::validation::facts::{
    AtsSigningServiceIdentifiedFact, AtsComplianceFact
};
...
fn test_ats_signing_service_identified_fact_properties() {
    let fact = AtsSigningServiceIdentifiedFact {
        is_ats_issued: true,
        issuer_cn: Some("Microsoft Trusted Signing CA".to_string()),
    ...
    if let Some(FactValue::Str(value)) = fact.get_property("issuer_cn") {
        assert_eq!(value, "Microsoft Trusted Signing CA");
    } else {
...
fn test_ats_compliance_fact_debug_and_clone() {
    ...
    assert!(debug_str.contains("AtsComplianceFact"));
...
fn test_ats_signing_service_identified_fact_debug_and_clone() {
    ...
    // Test Debug trait
    let debug_str = format!("{:?}", fact);
    assert!(debug_str.contains("AtsSigningServiceIdentifiedFact"));
