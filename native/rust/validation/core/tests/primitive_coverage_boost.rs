...
use cose_sign1_primitives::{CoseSign1Builder, CoseSign1Error, CoseSign1Message};
...
fn test_cbor_encoding_comprehensive() {
    ...
    encoder
        .encode_tstr("this is a longer string to test different encoding paths")
        .unwrap();
    ...
    for (_i, message_bytes) in test_messages.into_iter().enumerate() {
        ...
        match CoseSign1Message::parse(&message_bytes) {
            Ok(message) => {
                ...
                let pack = Arc::new(SimpleTrustPack::no_facts("test-parsing"))
                ...
                let _result = validator.validate_bytes(
                    ...
                    Arc::from(message_bytes.into_boxed_slice()),
                );
            }
            Err(_) => {
...
fn test_error_handling_comprehensive() {
    ...
    for input in malformed_inputs {
        ...
        let validator = CoseSign1Validator::new(vec![pack]);
        let _ =
            validator.validate_bytes(EverParseCborProvider, Arc::from(input.into_boxed_slice()));
    }
...
fn test_cose_headers_comprehensive() {
    ...
    for message_bytes in complex_messages {
        ...
        let pack =
            Arc::new(SimpleTrustPack::no_facts("header-test")) as Arc<dyn CoseSign1TrustPack>;
        let validator = CoseSign1Validator::new(vec![pack]);
        let _ = validator.validate_bytes(
            EverParseCborProvider,
            Arc::from(message_bytes.into_boxed_slice()),
        );
    }
