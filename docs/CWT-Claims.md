...
CoseSignTool generates DID:X509 identifiers in one of two formats, depending on the Enhanced Key Usage (EKU) extensions present in the leaf certificate:

#### Format 1: Non-Standard EKU (Preferred)
```
did:x509:0:sha256:{rootCertHash}::eku:{encodedEkuOid}
```

Used when the leaf certificate contains at least one non-standard EKU OID. If multiple non-standard EKUs are present, the OID is selected using the following priority:
1. **Arc depth** (number of dots) - descending: deeper OIDs are preferred (e.g., `1.1.1.1.1.1.1` has 6 dots)
2. **Last arc value** - numerically descending: when depths are equal, higher last arc values win (e.g., `1.2.3.999` > `1.2.3.500`)

Example: `1.1.1.1.1.1.1` (depth 6) is selected over `2.1` (depth 1), and `1.2.3.999` is selected over `1.2.3.100` (same depth).

**Standard EKU OIDs** (ignored for format selection):
...
**Example**:
```
did:x509:0:sha256:a1b2c3d4e5f6...::eku:1.3.6.1.4.1.311.10.3.13
```

#### Format 2: Subject-Based (Fallback)
```
did:x509:0:sha256:{rootCertHash}::subject:{encodedLeafSubject}
```

Used when the leaf certificate has no EKU extension, or only contains standard EKU OIDs.
...
**Note**: The examples above show the formats generated by CoseSignTool. The full DID:X509 specification supports additional query parameters beyond `::eku:` and `::subject:`. See the [DID:X509 specification](https://github.com/microsoft/did-x509/blob/main/specification.md) for complete details.
