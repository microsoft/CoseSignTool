...
impl AkvKeyClient {
    ...
    pub fn new_with_options(
        ...
        let kid_derived_version: Option<String> = key_response
            .key
            .as_ref()
        ...
        let resolved_version = key_version.map(|s| s.to_string()).or(kid_derived_version);
...
impl KeyVaultCryptoClient for AkvKeyClient {
    fn sign(&self, algorithm: &str, digest: &[u8]) -> Result<Vec<u8>, AkvError> {
        ...
        let result = self
            ...
            .block_on(async {
                self.client
                    .sign(&self.key_name, key_version, content, None)
                    .await
            })
        ...
        result
            .result
            .ok_or_else(|| AkvError::CryptoOperationFailed("no signature in response".into()))
    }
    ...
    fn public_key_bytes(&self) -> Result<Vec<u8>, AkvError> {
        ...
        Err(AkvError::General(
            "no public key components available (key may not have x/y for EC or n/e for RSA)"
                .into(),
        ))
    }
    fn name(&self) -> &str {
