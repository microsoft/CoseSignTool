...
///   The handle remains valid until freed via `cose_sign1_message_free()`.
...
/// - The pointer must be valid and non-null.
/// - The handle must not be freed while the returned reference is in use.
///   The handle remains valid until freed via `cose_key_free()`.
...
/// - The pointer must be valid and non-null.
/// - The handle must not be freed while the returned reference is in use.
///   The handle remains valid until freed via `cose_headermap_free()`.
