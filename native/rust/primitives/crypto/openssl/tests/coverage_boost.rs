...
fn test_cb_der_to_fixed_parses_two_byte_long_form_sequence() {
    ...
    assert_eq!(
        der[1], 0x82,
        "precondition: 2-byte long-form SEQUENCE length"
    );
...
fn test_cb_signer_from_der_unsupported_key_type_x25519() {
    ...
    assert!(
        result.is_err(),
        "X25519 should not be accepted as a signing key"
    );
}
...
fn test_cb_signer_ec_streaming_finalize_mismatched_algorithm() {
    ...
    assert!(
        result.is_err(),
        "finalize should fail with mismatched algorithm"
    );
}
...
fn test_cb_signer_ec_oneshot_mismatched_algorithm() {
    ...
    assert!(
        result.is_err(),
        "sign should fail with mismatched algorithm"
    );
}
...
fn test_cb_signer_streaming_rsa_pss_384_512() {
    ...
    for alg in [-38i64, -39] {
        ...
        assert!(
            valid,
            "PSS streaming signature for alg {} should verify",
            alg
        );
    }
}
