...
fn test_header_value_as_bytes_wrong_type() {
    ...
    let val = CoseHeaderValue::Text("hello".to_string().into());
    assert_eq!(val.as_bytes(), None);
...
fn test_header_value_as_bytes_correct_type() {
    let val = CoseHeaderValue::Bytes(vec![1, 2, 3].into());
...
fn test_header_value_as_str_correct_type() {
    let val = CoseHeaderValue::Text("hello world".to_string().into());
...
fn test_header_value_as_bytes_one_or_many_array_of_bytes() {
    let val = CoseHeaderValue::Array(vec![
        ...
        CoseHeaderValue::Bytes(vec![5, 6].into()),
    ]);
    let result = val.as_bytes_one_or_many();
...
fn test_header_value_as_bytes_one_or_many_array_mixed() {
    ...
    let val = CoseHeaderValue::Array(vec![
        ...
        CoseHeaderValue::Bytes(vec![3, 4].into()),
        CoseHeaderValue::Text("ignore".to_string().into()), // This should be ignored
    ]);
    let result = val.as_bytes_one_or_many();
...
fn test_header_map_get_bytes_one_or_many() {
    ...
    map.insert(
        ...
        CoseHeaderValue::Array(vec![
            CoseHeaderValue::Bytes(vec![4, 5].into()),
            CoseHeaderValue::Bytes(vec![6, 7].into()),
        ]),
    );
...
fn test_content_type_int_boundary() {
    ...
    map.insert(
        ...
        CoseHeaderValue::Int(u16::MAX as i64),
    );
...
fn test_crit_with_mixed_labels() {
    ...
    let crit_array = vec![
        ...
        CoseHeaderValue::Text("another".to_string().into()),
        CoseHeaderValue::Bytes(vec![1, 2].into()), // This should be filtered out
    ];
...
fn test_crit_wrong_type() {
    ...
    map.insert(
        CoseHeaderLabel::Int(CoseHeaderMap::CRIT),
        CoseHeaderValue::Text("not an array".to_string().into()),
    );
    assert_eq!(map.crit(), None);
}
...
fn test_header_map_iter() {
    ...
    let has_alg = items
        .iter()
        .any(|(k, _)| *k == &CoseHeaderLabel::Int(CoseHeaderMap::ALG));
    let has_kid = items
        .iter()
        .any(|(k, _)| *k == &CoseHeaderLabel::Int(CoseHeaderMap::KID));
    assert!(has_alg);
    assert!(has_kid);
}
...
fn test_protected_header_get() {
    ...
    assert_eq!(
        protected.get(&CoseHeaderLabel::Int(CoseHeaderMap::ALG)),
        Some(&CoseHeaderValue::Int(42))
    );
    assert_eq!(
        protected.get(&CoseHeaderLabel::Text("custom".to_string())),
