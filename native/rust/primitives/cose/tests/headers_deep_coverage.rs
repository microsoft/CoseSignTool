...
use cose_primitives::{
    ContentType, CoseError, CoseHeaderLabel, CoseHeaderMap, CoseHeaderValue, ProtectedHeader,
};
...
fn value_from_string() {
    assert_eq!(
        ...
        CoseHeaderValue::Text("s".to_string().into())
    );
}
...
fn value_from_str_ref() {
    assert_eq!(
        ...
        CoseHeaderValue::Text("r".to_string().into())
    );
}
...
#[test]
fn display_bytes() {
    let v = CoseHeaderValue::Bytes(vec![1, 2, 3].into());
...
fn display_bytes_empty() {
    assert_eq!(
        format!("{}", CoseHeaderValue::Bytes(vec![].into())),
        "bytes(0)"
    );
}
...
fn display_text() {
    assert_eq!(
        format!("{}", CoseHeaderValue::Text("abc".to_string().into())),
        "\"abc\""
    );
}
...
fn display_array_multiple() {
    let arr = CoseHeaderValue::Array(vec![
        ...
        CoseHeaderValue::Text("x".to_string().into()),
...
fn display_map_single() {
    let m = CoseHeaderValue::Map(vec![(
        ...
        CoseHeaderValue::Text("v".to_string().into()),
    )]);
...
#[test]
fn display_raw() {
    let r = CoseHeaderValue::Raw(vec![0xAA, 0xBB].into());
...
fn as_bytes_returns_some_for_bytes() {
    let v = CoseHeaderValue::Bytes(vec![1, 2].into());
...
fn as_bytes_returns_none_for_non_bytes() {
    ...
    assert!(CoseHeaderValue::Text("x".to_string().into())
        .as_bytes()
        .is_none());
}
...
fn as_str_returns_some() {
    let v = CoseHeaderValue::Text("abc".to_string().into());
...
fn as_bytes_one_or_many_array_of_bstr() {
    let v = CoseHeaderValue::Array(vec![
        CoseHeaderValue::Bytes(vec![0xAA].into()),
        CoseHeaderValue::Bytes(vec![0xBB].into()),
    ]);
    assert_eq!(v.as_bytes_one_or_many(), Some(vec![vec![0xAA], vec![0xBB]]));
}
...
fn map_get_bytes_one_or_many() {
    ...
    m.insert(label.clone(), CoseHeaderValue::Bytes(vec![1, 2, 3].into()));
...
fn encode_decode_text_value() {
    ...
    m.insert(
        ...
        CoseHeaderValue::Text("application/json".to_string().into()),
    );
    let decoded = CoseHeaderMap::decode(&m.encode().unwrap()).unwrap();
    assert_eq!(
...
fn encode_decode_tagged_value() {
    ...
    assert_eq!(
        ...
        Some(&CoseHeaderValue::Tagged(
            1,
            Box::new(CoseHeaderValue::Int(1234))
        ))
    );
}
...
fn encode_decode_raw_value() {
    ...
    m.insert(
        ...
        CoseHeaderValue::Raw(vec![0x18, 0x2a].into()),
    );
    let decoded = CoseHeaderMap::decode(&m.encode().unwrap()).unwrap();
    // Raw bytes decode as the underlying CBOR type, which is Int(42)
    assert_eq!(
        decoded.get(&CoseHeaderLabel::Int(40)),
...
fn encode_decode_array_of_mixed_types() {
    ...
    m.insert(
        ...
        CoseHeaderValue::Array(vec![
            ...
            CoseHeaderValue::Bytes(vec![0xFF].into()),
            CoseHeaderValue::Bool(true),
    ...
    m.insert(
        ...
        CoseHeaderValue::Map(vec![
            ...
            (
                ...
                CoseHeaderValue::Text("val".to_string().into()),
            ),
        ]),
    );
    let decoded = CoseHeaderMap::decode(&m.encode().unwrap()).unwrap();
...
fn encode_decode_large_int_label() {
    ...
    let decoded = CoseHeaderMap::decode(&m.encode().unwrap()).unwrap();
    assert_eq!(
        decoded.get(&CoseHeaderLabel::Int(1000)),
        Some(&CoseHeaderValue::Int(0))
    );
}
...
fn encode_decode_large_positive_value() {
    ...
    assert_eq!(
        decoded.get(&CoseHeaderLabel::Int(1)),
        Some(&CoseHeaderValue::Int(100_000))
    );
}
...
fn encode_decode_negative_int_label() {
    ...
    m.insert(
        CoseHeaderLabel::Int(-1),
        CoseHeaderValue::Text("neg".to_string().into()),
    );
    let decoded = CoseHeaderMap::decode(&m.encode().unwrap()).unwrap();
    assert_eq!(
        decoded.get(&CoseHeaderLabel::Int(-1)),
        Some(&CoseHeaderValue::Text("neg".to_string().into()))
    );
}
...
fn protected_header_get() {
    ...
    assert_eq!(
        ...
        Some(&CoseHeaderValue::Text("val".to_string().into()))
    );
...
fn encode_decode_deeply_nested_structure() {
    ...
    let inner_map = CoseHeaderValue::Map(vec![(CoseHeaderLabel::Int(99), inner_array)]);
    let outer_array = CoseHeaderValue::Array(vec![
        inner_map,
        CoseHeaderValue::Text("end".to_string().into()),
    ]);
