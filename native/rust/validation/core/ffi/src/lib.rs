...
pub extern "C" fn cose_sign1_validator_validate_bytes(
    ...
    with_catch_unwind(|| {
        ...
        // Convert the borrowed slice directly to Arc<[u8]> — single copy.
        // validate_bytes() will parse from this Arc, sharing the same allocation.
        let bytes: Arc<[u8]> = Arc::from(message);
        let r = v
