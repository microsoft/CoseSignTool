...
fn test_streaming_verify_es256_multiple_chunks() {
    ...
    sign_context
        .update(b"This is test ")
        .expect("should update");
    sign_context
        .update(b"data for streaming ")
        .expect("should update");
    ...
    let mut verify_context = verifier
        .verify_init(&signature)
        .expect("should create verify context");
    verify_context
        .update(b"This is test ")
        .expect("should update");
    verify_context
...
fn test_streaming_verify_es384_different_chunk_sizes() {
    ...
    let mut sign_context = signer.sign_init().expect("should create sign context");
    sign_context
        .update(&test_data[0..20])
        .expect("should update");
    sign_context
        .update(&test_data[20..50])
        .expect("should update");
    sign_context
        .update(&test_data[50..])
        .expect("should update");
    ...
    let mut verify_context = verifier
        ...
        .expect("should create verify context");
    verify_context
        .update(&test_data[0..10])
        .expect("should update");
    verify_context
        .update(&test_data[10..40])
        .expect("should update");
    verify_context
        .update(&test_data[40..60])
        .expect("should update");
    verify_context
        .update(&test_data[60..])
        .expect("should update");
    ...
    assert!(
        result,
        "verification should succeed despite different chunking"
    );
}
...
fn test_streaming_verify_rsa_pss_all_algorithms() {
    ...
    for (alg, name) in [(-37, "PS256"), (-38, "PS384"), (-39, "PS512")] {
        let signer = EvpSigner::from_der(&private_der, alg)
            .expect(&format!("should create {} signer", name));
        let verifier = EvpVerifier::from_der(&public_der, alg)
            .expect(&format!("should create {} verifier", name));
...
fn test_streaming_verify_rsa_pkcs1_all_algorithms() {
    ...
    for (alg, name) in [(-257, "RS256"), (-258, "RS384"), (-259, "RS512")] {
        ...
        for &byte in test_bytes {
            verify_context
                .update(&[byte])
                .expect("should update single byte");
        }
        let result = verify_context.finalize().expect("should finalize");

        assert!(
            result,
            "{} single-byte streaming verification should succeed",
            name
        );
    }
}
...
fn test_streaming_verify_ed25519_empty_updates() {
    ...
    assert!(
        !signer.supports_streaming(),
        "ED25519 signer should not support streaming"
    );
    assert!(
        !verifier.supports_streaming(),
        "ED25519 verifier should not support streaming"
    );
}
...
fn test_streaming_verify_supports_streaming() {
    ...
    assert!(
        verifier.supports_streaming(),
        "verifier should support streaming"
    );
...
fn test_streaming_verify_key_cloning() {
    ...
    let context1 = verifier
        .verify_init(&signature)
        .expect("should create context 1");
    let context2 = verifier
        .verify_init(&signature)
        .expect("should create context 2");
...
fn test_streaming_verify_mixed_with_oneshot() {
    ...
    let another_result = verifier
        .verify(test_data, &signature)
        .expect("should verify again");
    assert!(
        another_result,
        "second one-shot verification should succeed"
    );
}
...
fn test_streaming_verify_different_signature_chunk_alignment() {
    ...
    assert!(
        result,
        "verification with different chunk alignment should succeed"
    );
}
