...
use azure_trusted_signing_client::error::*;
use azure_trusted_signing_client::models::*;
...
fn error_display_all_variants() {
    assert_eq!(format!("{}", AtsClientError::HttpError("timeout".into())), "HTTP error: timeout");
    assert_eq!(format!("{}", AtsClientError::AuthenticationFailed("bad token".into())), "Authentication failed: bad token");
    assert_eq!(format!("{}", AtsClientError::DeserializationError("bad json".into())), "Deserialization error: bad json");
    assert_eq!(format!("{}", AtsClientError::InvalidConfiguration("missing".into())), "Invalid configuration: missing");
    assert_eq!(format!("{}", AtsClientError::CertificateChainNotAvailable("none".into())), "Certificate chain not available: none");
    assert_eq!(format!("{}", AtsClientError::SignFailed("err".into())), "Sign failed: err");
    assert_eq!(format!("{}", AtsClientError::OperationTimeout { operation_id: "op1".into() }), "Operation op1 timed out");
    assert_eq!(format!("{}", AtsClientError::OperationFailed { operation_id: "op2".into(), status: "Failed".into() }), "Operation op2 failed with status: Failed");

    let with_target = AtsClientError::ServiceError { code: "E01".into(), message: "bad".into(), target: Some("res".into()) };
    assert!(format!("{}", with_target).contains("(target: res)"));
    let no_target = AtsClientError::ServiceError { code: "E01".into(), message: "bad".into(), target: None };
    assert!(!format!("{}", no_target).contains("target"));
}

#[test]
fn error_is_std_error() {
    let err: Box<dyn std::error::Error> = Box::new(AtsClientError::HttpError("x".into()));
