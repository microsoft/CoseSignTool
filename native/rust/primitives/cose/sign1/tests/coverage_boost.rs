...
impl CryptoVerifier for MockVerifier {
    ...
    fn verify(&self, data: &[u8], signature: &[u8]) -> Result<bool, CryptoError> {
        ...
        let expected_len =
            u32::from_be_bytes([signature[2], signature[3], signature[4], signature[5]]);
...
fn test_builder_sign_streaming_non_streaming_signer() {
    ...
    let bytes = result.unwrap();
    let msg = CoseSign1Message::parse(&bytes).expect("parse");
    assert_eq!(msg.payload(), Some(b"streaming test".as_slice()));
}
...
fn test_builder_sign_streaming_with_streaming_signer() {
    ...
    let bytes = result.unwrap();
    let msg = CoseSign1Message::parse(&bytes).expect("parse");
    assert_eq!(msg.payload(), Some(b"streaming signer test".as_slice()));
}
...
fn test_builder_with_unprotected_headers() {
    ...
    unprotected.insert(
        ...
        cose_sign1_primitives::CoseHeaderValue::Bytes(b"kid-value".to_vec().into()),
    );
...
fn test_message_parse_and_verify_roundtrip() {
    ...
    let msg = CoseSign1Message::parse(&bytes).expect("parse");
    assert_eq!(msg.alg(), Some(-7));
    assert_eq!(msg.payload(), Some(b"verify me".as_slice()));
    assert!(!msg.signature().is_empty());

    // Verify
    let valid = msg
        .verify(&MockVerifier, None)
...
fn test_message_verify_with_external_aad() {
    ...
    let valid = msg.verify(&MockVerifier, Some(b"context")).expect("verify");
    assert!(valid);
...
fn test_sig_structure_hasher() {
    ...
    hasher.init(b"protected-bytes", None, 5).expect("init");
...
fn test_hash_sig_structure_streaming() {
    ...
    let result = hash_sig_structure_streaming(WriteCollector::new(), b"protected", None, sized);
...
fn test_stream_sig_structure() {
    ...
    let result = stream_sig_structure(&mut output, b"protected", None, sized);
