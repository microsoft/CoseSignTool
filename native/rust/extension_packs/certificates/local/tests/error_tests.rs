...
fn test_key_generation_failed_display() {
    ...
    assert_eq!(
        display_str,
        "key generation failed: RSA key generation failed"
    );
}
...
fn test_certificate_creation_failed_display() {
    ...
    assert_eq!(
        display_str,
        "certificate creation failed: X.509 encoding failed"
    );
}
...
fn test_all_error_variants_display() {
    ...
    for (error, expected_prefix) in errors.iter().zip(expected_prefixes.iter()) {
        let display_str = format!("{}", error);
        assert!(
            display_str.starts_with(expected_prefix),
    ...
    for i in 0..displays.len() {
        for j in i + 1..displays.len() {
            assert_ne!(
                ...
                "Error variants {} and {} should have different displays",
                i, j
            );
        }
    }
