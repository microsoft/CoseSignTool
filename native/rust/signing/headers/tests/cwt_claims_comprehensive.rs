...
fn test_cwt_claims_to_cbor_bytes_full() {
    ...
    assert!(
        cbor_bytes.len() > 10,
        "CBOR bytes should contain substantial data"
    );
}
...
fn test_cwt_claims_partial_fields() {
    ...
    assert!(
        result.is_ok(),
        "Partial claims CBOR encoding should succeed"
    );

    let cbor_bytes = result.unwrap();
    assert!(
        !cbor_bytes.is_empty(),
        "Partial CBOR bytes should not be empty"
    );
}
...
fn test_cwt_claims_with_custom_claims() {
    ...
    claims
        .custom_claims
        .insert(100, CwtClaimValue::Text("custom-value".to_string()));
    ...
    claims
        .custom_claims
        .insert(103, CwtClaimValue::Bytes(b"binary-data".to_vec()));
    ...
    assert!(
        result.is_ok(),
        "Claims with custom values should encode successfully"
    );
    ...
    assert_eq!(
        ...
        Some(&CwtClaimValue::Bytes(b"binary-data".to_vec()))
    );
}
...
fn test_cwt_claims_edge_cases() {
    ...
    assert!(
        result.is_ok(),
        "Empty string claims should encode successfully"
    );
...
fn test_cwt_claims_boundary_times() {
    ...
    assert!(
        result.is_ok(),
        "Epoch timestamp claims should encode successfully"
    );
    ...
    assert!(
        result.is_ok(),
        "Max timestamp claims should encode successfully"
    );
}
...
fn test_cwt_claims_large_custom_data() {
    ...
    claims
        .custom_claims
        .insert(200, CwtClaimValue::Text(large_string.clone()));

    // Add large binary custom claim
    let large_binary = vec![0x42; 5000];
    claims
        .custom_claims
        .insert(201, CwtClaimValue::Bytes(large_binary.clone()));

    // Test encoding with large data
    let result = claims.to_cbor_bytes();
    assert!(
        result.is_ok(),
        "Large custom claims should encode successfully"
    );

    // Verify data integrity
    assert_eq!(
        claims.custom_claims.get(&200),
        Some(&CwtClaimValue::Text(large_string))
    );
    assert_eq!(
        claims.custom_claims.get(&201),
        Some(&CwtClaimValue::Bytes(large_binary))
    );
}
...
fn test_cwt_claims_unicode_strings() {
    ...
    assert!(
        result.is_ok(),
        "Unicode string claims should encode successfully"
    );
...
fn test_cwt_claims_binary_id() {
    ...
    for (i, pattern) in binary_patterns.iter().enumerate() {
        ...
        assert!(
            result.is_ok(),
            "Binary pattern {} should encode successfully",
            i
        );
        ...
        assert_eq!(
            claims.cwt_id,
            Some(pattern.clone()),
            "Binary pattern {} should be preserved",
            i
        );
    }
}
...
fn test_cwt_claims_claim_key_ranges() {
    ...
    claims
        .custom_claims
        .insert(1000, CwtClaimValue::Text("positive".to_string()));
    claims
        .custom_claims
        .insert(i64::MAX, CwtClaimValue::Integer(42));

    // Negative keys
    claims.custom_claims.insert(-1, CwtClaimValue::Bool(true));
    claims
        .custom_claims
        .insert(i64::MIN, CwtClaimValue::Integer(42));

    // Zero key
    claims
        .custom_claims
        .insert(0, CwtClaimValue::Bytes(b"zero".to_vec()));
    ...
    assert!(
        result.is_ok(),
        "Various claim key ranges should encode successfully"
    );
...
fn test_cwt_claims_clone_and_modify() {
    ...
    assert_eq!(
        modified.issuer,
        Some("https://modified.example".to_string())
    );
...
fn test_cwt_claims_concurrent_modification() {
    ...
    for i in 0..4 {
        assert_eq!(
            final_claims.custom_claims.get(&i),
...
fn test_cwt_claims_memory_efficiency() {
    ...
    for i in 0..100 {
        large_claims
            .custom_claims
            .insert(i, CwtClaimValue::Integer(i));
    }
