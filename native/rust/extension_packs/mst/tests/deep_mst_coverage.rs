...
fn error_display_receipt_decode() {
    let e = ReceiptVerifyError::ReceiptDecode(Cow::Borrowed("bad cbor"));
    let s = format!("{}", e);
...
fn error_display_jwks_parse() {
    let e = ReceiptVerifyError::JwksParse(Cow::Borrowed("bad json"));
    assert!(format!("{}", e).contains("jwks_parse_failed"));
}

#[test]
fn error_display_jwks_fetch() {
    let e = ReceiptVerifyError::JwksFetch(Cow::Borrowed("network error"));
    assert!(format!("{}", e).contains("jwks_fetch_failed"));
}

#[test]
fn error_display_jwk_not_found() {
    let e = ReceiptVerifyError::JwkNotFound(Cow::Borrowed("kid123"));
...
fn error_display_jwk_unsupported() {
    let e = ReceiptVerifyError::JwkUnsupported(Cow::Borrowed("rsa"));
    assert!(format!("{}", e).contains("jwk_unsupported"));
}

#[test]
fn error_display_statement_reencode() {
    let e = ReceiptVerifyError::StatementReencode(Cow::Borrowed("cbor fail"));
    assert!(format!("{}", e).contains("statement_reencode_failed"));
}

#[test]
fn error_display_sig_structure_encode() {
    let e = ReceiptVerifyError::SigStructureEncode(Cow::Borrowed("sig fail"));
...
fn error_is_std_error() {
    ...
    let e: Box<dyn std::error::Error> = Box::new(ReceiptVerifyError::MissingAlg);
...
fn extract_proof_blobs_proof_not_array() {
    ...
    let pairs = vec![(CoseHeaderLabel::Int(-1), CoseHeaderValue::Int(99))];
...
fn extract_proof_blobs_empty_proof_array() {
    ...
    let pairs = vec![(CoseHeaderLabel::Int(-1), CoseHeaderValue::Array(vec![]))];
    let value = CoseHeaderValue::Map(pairs);
    let result = extract_proof_blobs(&value);
    assert!(result.is_err());
    let msg = format!("{}", result.unwrap_err());
...
fn jwk_to_spki_p256_valid() {
    ...
    assert!(
        result.is_ok(),
        "Valid P-256 JWK should produce EcJwk: {:?}",
        result.err()
    );
    let ec = result.unwrap();
    assert_eq!(ec.kty, "EC");
...
fn jwk_to_spki_p384_valid() {
    ...
    assert!(
        result.is_ok(),
        "Valid P-384 JWK should produce EcJwk: {:?}",
        result.err()
    );
    let ec = result.unwrap();
    assert_eq!(ec.kty, "EC");
...
fn get_cwt_issuer_host_non_map_value() {
    ...
    let result = get_cwt_issuer_host(protected.headers(), 15, 1);
...
fn get_cwt_issuer_host_map_without_iss() {
    ...
    let inner_pairs = vec![(
        ...
        CoseHeaderValue::Text("test-subject".to_string().into()),
    )];
    let mut hdr = cose_sign1_primitives::CoseHeaderMap::new();
...
fn get_cwt_issuer_host_found() {
    let inner_pairs = vec![(
        CoseHeaderLabel::Int(1),
        CoseHeaderValue::Text("example.ledger.azure.net".to_string().into()),
    )];
    let mut hdr = cose_sign1_primitives::CoseHeaderMap::new();
    hdr.insert(CoseHeaderLabel::Int(15), CoseHeaderValue::Map(inner_pairs));
    let protected = cose_sign1_primitives::ProtectedHeader::encode(hdr).unwrap();
