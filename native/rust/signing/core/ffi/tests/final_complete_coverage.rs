...
unsafe extern "C" fn tracked_sign_callback(
    ...
    if !sig_structure.is_null() && sig_structure_len > 0 {
        let data_slice =
            unsafe { std::slice::from_raw_parts(sig_structure, sig_structure_len.min(16)) };
...
unsafe extern "C" fn failing_after_n_calls_callback(
    ...
    let max_calls = if user_data.is_null() {
        2
    } else {
        user_data as usize
    };
...
unsafe extern "C" fn boundary_read_callback(
    ...
    static BOUNDARY_CALL_COUNT: std::sync::atomic::AtomicUsize =
...
fn create_callback_key_with_user_data(
    algorithm: i64,
    key_type: &str,
    max_calls: usize,
) -> *mut CoseKeyHandle {
...
fn test_callback_streaming_payload_size_and_open_methods() {
    ...
    let test_sizes = vec![
        0u64, 1, 2, 7, 8, 15, 16, 31, 32, 63, 64, 127, 128, 255, 256, 511, 512, 1023, 1024, 2047,
