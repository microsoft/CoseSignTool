...
fn display_all_header_value_variants() {
    ...
    // Raw (line 158)
    assert_eq!(
        format!("{}", CoseHeaderValue::Raw(vec![0xAA, 0xBB].into())),
        "raw(2)"
    );
}
...
fn encode_decode_roundtrip_all_value_types() {
    ...
    map.insert(
        ...
        CoseHeaderValue::Array(vec![
            ...
            CoseHeaderValue::Bytes(vec![2].into()),
        ]),
    );
    ...
    match decoded.get(&CoseHeaderLabel::Int(30)) {
        Some(CoseHeaderValue::Array(arr)) => {
            assert_eq!(arr.len(), 2);
            assert_eq!(arr[0], CoseHeaderValue::Bytes(vec![1].into()));
            assert_eq!(arr[1], CoseHeaderValue::Bytes(vec![2].into()));
        }
...
fn protected_header_encode_decode_roundtrip() {
    ...
    let decoded = ProtectedHeader::decode(protected.as_bytes().to_vec()).expect("decode protected");
...
fn header_value_as_bytes_returns_some_for_bytes() {
    let val = CoseHeaderValue::Bytes(vec![1, 2].into());
    ...
    outer.insert(
        ...
        CoseHeaderValue::Map(vec![
            ...
            (
                ...
                CoseHeaderValue::Bytes(vec![0xBE, 0xEF].into()),
            ),
        ]),
    );
