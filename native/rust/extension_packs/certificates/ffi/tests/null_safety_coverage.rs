...
#[test]
fn test_cose_sign1_certificates_key_from_cert_der_null_safety() {
    // Test null certificate pointer
    let mut key: *mut cose_sign1_primitives_ffi::types::CoseKeyHandle = ptr::null_mut();
    let result = cose_sign1_certificates_key_from_cert_der(ptr::null(), 0, &mut key);
    assert_ne!(result, cose_status_t::COSE_OK); // Should fail

    // Test null output pointer
    let test_data = b"test";
    let result = cose_sign1_certificates_key_from_cert_der(
        test_data.as_ptr(),
        test_data.len(),
        ptr::null_mut(),
    );
    ...
    let mut key: *mut cose_sign1_primitives_ffi::types::CoseKeyHandle = ptr::null_mut();
    let result = cose_sign1_certificates_key_from_cert_der(test_data.as_ptr(), 0, &mut key);
    assert_ne!(result, cose_status_t::COSE_OK); // Should fail
...
fn test_trust_policy_builder_functions_null_safety() {
    ...
    let result = cose_sign1_certificates_trust_policy_builder_require_x509_chain_not_trusted(
        ptr::null_mut(),
    );
    assert_ne!(result, cose_status_t::COSE_OK);
    ...
    let result = cose_sign1_certificates_trust_policy_builder_require_x509_chain_element_count_eq(
        ptr::null_mut(),
        1,
    );
    assert_ne!(result, cose_status_t::COSE_OK);

    let result = cose_sign1_certificates_trust_policy_builder_require_x509_chain_status_flags_eq(
        ptr::null_mut(),
        0,
    );
    assert_ne!(result, cose_status_t::COSE_OK);

    let result = cose_sign1_certificates_trust_policy_builder_require_leaf_chain_thumbprint_present(
        ptr::null_mut(),
    );
    assert_ne!(result, cose_status_t::COSE_OK);

    let result = cose_sign1_certificates_trust_policy_builder_require_signing_certificate_present(
        ptr::null_mut(),
    );
    assert_ne!(result, cose_status_t::COSE_OK);
}
...
fn test_validator_builder_with_certificates_pack_null_safety() {
    ...
    let result =
        cose_sign1_validator_builder_with_certificates_pack_ex(ptr::null_mut(), ptr::null());
    assert_ne!(result, cose_status_t::COSE_OK);
