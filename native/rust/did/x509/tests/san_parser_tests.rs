...
use did_x509::models::{SanType, SubjectAlternativeName};
...
fn test_parse_san_extension_with_mock_extension() {
    ...
    let _result = parse_san_extension(&X509Extension::new(
        oid.clone(),
        false,
        ext_data,
...
fn test_parse_san_extension_invalid() {
    ...
    assert_eq!(
        result.unwrap_err(),
        "Extension is not a SubjectAlternativeName"
    );
}
...
fn test_parse_sans_no_extensions() {
    ...
    let _ =
        parse_san_extension as fn(&X509Extension) -> Result<Vec<SubjectAlternativeName>, String>;
