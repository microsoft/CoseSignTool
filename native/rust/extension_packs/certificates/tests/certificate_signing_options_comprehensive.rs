...
fn test_default_options() {
    ...
    assert_eq!(
        ...
        "SCITT compliance should be enabled by default"
    );
    assert!(
        options.custom_cwt_claims.is_none(),
        "Custom CWT claims should be None by default"
    );
}
...
fn test_new_equals_default() {
    ...
    assert_eq!(
        new_opts.enable_scitt_compliance,
        default_opts.enable_scitt_compliance
    );
    assert_eq!(
        new_opts.custom_cwt_claims.is_none(),
        default_opts.custom_cwt_claims.is_none()
    );
}
...
fn test_disable_scitt_compliance() {
    ...
    assert_eq!(
        options.enable_scitt_compliance, false,
        "Should allow disabling SCITT compliance"
    );
}
...
fn test_enable_scitt_compliance() {
    ...
    assert_eq!(
        options.enable_scitt_compliance, true,
        "Should allow re-enabling SCITT compliance"
    );
}
...
fn test_set_custom_cwt_claims() {
    ...
    assert!(
        options.custom_cwt_claims.is_some(),
        "Should allow setting custom CWT claims"
    );
    assert_eq!(
...
fn test_clear_custom_cwt_claims() {
    ...
    assert!(
        options.custom_cwt_claims.is_none(),
        "Should allow clearing custom CWT claims"
    );
}
