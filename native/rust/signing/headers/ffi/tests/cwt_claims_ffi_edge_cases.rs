...
fn test_cwt_claims_to_cbor() {
    unsafe {
        ...
        let issuer = CString::new("test-issuer").unwrap();
        assert_eq!(
            cose_cwt_claims_set_issuer(handle, issuer.as_ptr(), &mut error),
            COSE_CWT_OK
        );

        let subject = CString::new("test.subject").unwrap();
        assert_eq!(
            cose_cwt_claims_set_subject(handle, subject.as_ptr(), &mut error),
            COSE_CWT_OK
        );
...
fn test_cwt_claims_to_cbor_null_handle() {
    unsafe {
        ...
        let mut error = ptr::null_mut();
        let status =
            cose_cwt_claims_to_cbor(ptr::null_mut(), &mut out_ptr, &mut out_len, &mut error);
...
fn test_cwt_claims_all_setters_null_handle() {
    unsafe {
        ...
        assert_eq!(
            cose_cwt_claims_set_issuer(ptr::null_mut(), test_string.as_ptr(), &mut error),
            COSE_CWT_ERR_NULL_POINTER
        );
        assert_eq!(
            cose_cwt_claims_set_subject(ptr::null_mut(), test_string.as_ptr(), &mut error),
            COSE_CWT_ERR_NULL_POINTER
        );
        assert_eq!(
            cose_cwt_claims_set_audience(ptr::null_mut(), test_string.as_ptr(), &mut error),
            COSE_CWT_ERR_NULL_POINTER
        );
        assert_eq!(
            cose_cwt_claims_set_expiration(ptr::null_mut(), 1000, &mut error),
            COSE_CWT_ERR_NULL_POINTER
        );
        assert_eq!(
            cose_cwt_claims_set_not_before(ptr::null_mut(), 500, &mut error),
            COSE_CWT_ERR_NULL_POINTER
        );
        assert_eq!(
            cose_cwt_claims_set_issued_at(ptr::null_mut(), 750, &mut error),
            COSE_CWT_ERR_NULL_POINTER
        );
...
fn test_cwt_claims_comprehensive_workflow() {
    unsafe {
        ...
        assert_eq!(
            cose_cwt_claims_set_audience(handle, audience.as_ptr(), &mut error),
            COSE_CWT_OK
        );

        assert_eq!(
            cose_cwt_claims_set_expiration(handle, 2000000000, &mut error),
            COSE_CWT_OK
        );
        assert_eq!(
            cose_cwt_claims_set_not_before(handle, 1500000000, &mut error),
            COSE_CWT_OK
        );
        assert_eq!(
            cose_cwt_claims_set_issued_at(handle, 1600000000, &mut error),
            COSE_CWT_OK
        );
...
fn test_cwt_claims_zero_length_strings() {
    unsafe {
        ...
        assert_eq!(
            cose_cwt_claims_set_issuer(handle, empty_string.as_ptr(), &mut error),
            COSE_CWT_OK
        );
        assert_eq!(
            cose_cwt_claims_set_subject(handle, empty_string.as_ptr(), &mut error),
            COSE_CWT_OK
        );
        assert_eq!(
            cose_cwt_claims_set_audience(handle, empty_string.as_ptr(), &mut error),
            COSE_CWT_OK
        );
...
fn test_cwt_claims_from_cbor_roundtrip() {
    unsafe {
        ...
        assert_eq!(
            cose_cwt_claims_to_cbor(handle, &mut cbor_ptr, &mut cbor_len, &mut error),
            COSE_CWT_OK
        );
        ...
        let mut out_issuer: *const libc::c_char = ptr::null();
        assert_eq!(
            cose_cwt_claims_get_issuer(handle2, &mut out_issuer, &mut error),
            COSE_CWT_OK
        );
        if !out_issuer.is_null() {
            let retrieved = std::ffi::CStr::from_ptr(out_issuer);
