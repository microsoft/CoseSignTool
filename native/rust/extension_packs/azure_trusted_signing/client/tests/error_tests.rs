...
use azure_trusted_signing_client::AtsClientError;
use std::error::Error;

#[test]
fn test_http_error_display() {
    let error = AtsClientError::HttpError("Network timeout".to_string());
...
fn test_authentication_failed_display() {
    let error = AtsClientError::AuthenticationFailed("Token expired".to_string());
...
fn test_service_error_display_with_target() {
    let error = AtsClientError::ServiceError {
...
fn test_operation_failed_display() {
    let error = AtsClientError::OperationFailed {
...
fn test_operation_timeout_display() {
    let error = AtsClientError::OperationTimeout {
...
fn test_deserialization_error_display() {
    let error = AtsClientError::DeserializationError("Invalid JSON".to_string());
...
fn test_invalid_configuration_display() {
    let error = AtsClientError::InvalidConfiguration("Missing endpoint".to_string());
...
fn test_certificate_chain_not_available_display() {
    let error = AtsClientError::CertificateChainNotAvailable("Chain expired".to_string());
...
fn test_sign_failed_display() {
    let error = AtsClientError::SignFailed("Signing service unavailable".to_string());
...
fn test_error_trait_implementation() {
    let error = AtsClientError::HttpError("Test error".to_string());
...
fn test_all_variants_implement_error_trait() {
    let errors: Vec<Box<dyn Error>> = vec![
        Box::new(AtsClientError::HttpError("test".to_string())),
        Box::new(AtsClientError::AuthenticationFailed("test".to_string())),
        Box::new(AtsClientError::ServiceError {
            code: "test".to_string(),
            message: "test".to_string(),
            target: None,
        }),
        Box::new(AtsClientError::OperationFailed {
            operation_id: "test".to_string(),
            status: "test".to_string(),
        }),
        Box::new(AtsClientError::OperationTimeout {
            operation_id: "test".to_string(),
        }),
        Box::new(AtsClientError::DeserializationError("test".to_string())),
        Box::new(AtsClientError::InvalidConfiguration("test".to_string())),
        Box::new(AtsClientError::CertificateChainNotAvailable("test".to_string())),
        Box::new(AtsClientError::SignFailed("test".to_string())),
    ];
