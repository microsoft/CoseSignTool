...
fn test_header_value_variants() {
    ...
    let raw_val = CoseHeaderValue::Raw(vec![0xa1, 0x00, 0x01].into());
...
fn test_header_value_conversions() {
    ...
    assert_eq!(from_vec_u8, CoseHeaderValue::Bytes(vec![1, 2, 3].into()));

    let from_slice: CoseHeaderValue = [1, 2, 3].as_slice().into();
    assert_eq!(from_slice, CoseHeaderValue::Bytes(vec![1, 2, 3].into()));
    ...
    assert_eq!(from_str, CoseHeaderValue::Text("test".to_string().into()));
...
fn test_header_value_as_bytes_one_or_many() {
    // Single bytes value
    let single = CoseHeaderValue::Bytes(vec![1, 2, 3].into());
...
fn test_header_value_accessors() {
    ...
    let non_int = CoseHeaderValue::Text("hello".to_string().into());
    ...
    let complex = CoseHeaderValue::Map(vec![
        (
            ...
            CoseHeaderValue::Array(vec![
                CoseHeaderValue::Int(1),
                CoseHeaderValue::Text("test".to_string().into()),
                CoseHeaderValue::Tagged(42, Box::new(CoseHeaderValue::Bytes(vec![1, 2, 3].into()))),
            ]),
        ),
        (
            CoseHeaderLabel::Text("nested".to_string()),
