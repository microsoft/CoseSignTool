...
) -> Result<AkvKeyClient, cose_sign1_azure_key_vault::common::error::AkvError> {
...
fn new_with_options_sign_success() {
    ...
    let client =
        mock_akv_client(vec![MockResponse::ok(get_key), MockResponse::ok(sign_resp)]).unwrap();
...
fn public_key_bytes_ec_returns_uncompressed_point() {
    ...
    assert!(
        result.is_ok(),
        "public_key_bytes should succeed for EC key: {:?}",
        result.err()
    );
    let bytes = result.unwrap();
    assert_eq!(
        bytes[0], 0x04,
        "EC public key should start with 0x04 (uncompressed)"
    );
    assert_eq!(
        bytes.len(),
        1 + 32 + 32,
        "P-256 uncompressed point = 1 + 32 + 32 bytes"
    );
}
