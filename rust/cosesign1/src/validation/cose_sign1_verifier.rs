...
use crate::common::{encode_signature1_sig_structure, parse_cose_sign1, ParsedCoseSign1};
...
pub fn verify_cose_sign1(validator_name: &str, cose_sign1: &[u8], options: &VerifyOptions) -> ValidationResult {
    let parsed = match parse_cose_sign1(cose_sign1) {
        Ok(p) => p,
        Err(e) => {
            return ValidationResult::failure_message(validator_name, e, Some("COSE_PARSE_ERROR".to_string()))
        }
    };
...
pub fn verify_parsed_cose_sign1_detached_payload_reader(
    ...
    let res = verify_sig_structure_detached_reader(alg, key_bytes, parsed, payload_reader, &parsed.signature);
    match res {
        Ok(()) => ValidationResult::success(validator_name, Default::default()),
        Err((code, msg)) => ValidationResult::failure_message(validator_name, msg, Some(code)),
    }
...
fn stream_into_digest(reader: &mut dyn crate::ReadSeek, digest: &mut dyn FnMut(&[u8])) -> Result<(), String> {
    let mut buf = [0u8; 64 * 1024];
    loop {
        let n = reader.read(&mut buf).map_err(|e| format!("failed to read detached payload: {e}"))?;
        if n == 0 {
            break;
        }
        digest(&buf[..n]);
...
fn verify_sig_structure_detached_reader(
    ...
    let payload_len = end
        .checked_sub(start)
        .ok_or_else(|| ("SIGSTRUCT_ERROR".to_string(), "invalid payload stream position".to_string()))?;
    payload_reader
...
fn verify_ecdsa_p256_prehash(pub_bytes: &[u8], prehash: &[u8], sig: &[u8]) -> Result<(), (String, String)> {
    let spki = extract_spki_der_from_der_key_or_cert(pub_bytes)?;
    let pk = p256::PublicKey::from_public_key_der(&spki)
        .map_err(|e| ("INVALID_PUBLIC_KEY".to_string(), format!("bad P-256 public key: {e}")))?;
...
fn verify_ecdsa_p384_prehash(pub_bytes: &[u8], prehash: &[u8], sig: &[u8]) -> Result<(), (String, String)> {
    let spki = extract_spki_der_from_der_key_or_cert(pub_bytes)?;
    let pk = p384::PublicKey::from_public_key_der(&spki)
        .map_err(|e| ("INVALID_PUBLIC_KEY".to_string(), format!("bad P-384 public key: {e}")))?;
    let ep = pk.to_encoded_point(false);
...
fn verify_ecdsa_p521_prehash(pub_bytes: &[u8], prehash: &[u8], sig: &[u8]) -> Result<(), (String, String)> {
    let spki = extract_spki_der_from_der_key_or_cert(pub_bytes)?;
    let pk = p521::PublicKey::from_public_key_der(&spki)
        .map_err(|e| ("INVALID_PUBLIC_KEY".to_string(), format!("bad P-521 public key: {e}")))?;
    let ep = pk.to_encoded_point(false);
...
fn verify_rsa_pss_prehash(pub_bytes: &[u8], prehash: &[u8], sig: &[u8]) -> Result<(), (String, String)> {
    let key = rsa_public_key(pub_bytes)?;
