...
// CborProvider must be in scope for the `.encoder()` trait method on the provider.
...
impl CoseSign1Builder {
    ...
    pub fn sign_streaming(
        ...
        // Sign the payload, capturing bytes for embedding if needed (avoids re-reading).
        let (signature, buffered_payload) = if signer.supports_streaming() {
            ...
            if self.detached {
                ...
                let expected_len = usize::try_from(payload_len).map_err(|_| {
                    CoseSign1Error::PayloadError(crate::error::PayloadError::LengthMismatch {
                ...
                if total_read != expected_len {
                    return Err(CoseSign1Error::PayloadError(
                        crate::error::PayloadError::LengthMismatch {
                            expected: payload_len,
                            actual: total_read as u64,
                        },
                    ));
                }
                ...
                let capacity = usize::try_from(payload_len).map_err(|_| {
                    CoseSign1Error::PayloadTooLargeForEmbedding(payload_len, usize::MAX as u64)
                })?;
            ...
            std::io::Read::read_to_end(reader.as_mut(), &mut payload_bytes)
                .map_err(|e| CoseSign1Error::IoError(e.to_string()))?;
            let sig_structure =
                build_sig_structure(&protected_bytes, external_aad, &payload_bytes)?;
            let sig = signer.sign(&sig_structure).map_err(CoseKeyError::from)?;
            // Reuse the already-buffered payload for embedding instead of re-reading.
            (
                sig,
                if self.detached {
                    None
        ...
        self.build_message_opt(protected_bytes, buffered_payload.as_deref(), signature)
    }
