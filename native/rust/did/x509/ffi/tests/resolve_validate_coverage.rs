...
fn test_resolve_inner_invalid_did() {
    ...
    assert!(
        err_msg.contains("must start with 'did:x509'"),
        "Error: {}",
        err_msg
    );
...
fn test_validate_inner_wrong_chain() {
    ...
    assert_ne!(
        is_valid, 1,
        "Certificate should not be valid for the mismatched DID"
    );

    let err_msg = error_message(error).unwrap();
    assert!(
        err_msg.contains("fingerprint"),
...
fn test_build_from_chain_invalid_cert() {
    ...
    let err_msg = error_message(error).unwrap();
    assert!(
        err_msg.contains("parse") || err_msg.contains("build") || err_msg.contains("invalid"),
        "Error: {}",
        err_msg
    );
