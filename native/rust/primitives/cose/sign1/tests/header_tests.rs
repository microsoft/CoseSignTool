...
fn test_header_value_bytes() {
    ...
    let value = CoseHeaderValue::Bytes(bytes.clone().into());
    assert_eq!(value, CoseHeaderValue::Bytes(bytes.clone().into()));
...
fn test_header_value_text() {
    let text = "hello";
    let value = CoseHeaderValue::Text(text.to_string().into());
    assert_eq!(value, CoseHeaderValue::Text(text.to_string().into()));
...
fn test_header_value_raw() {
    ...
    let value = CoseHeaderValue::Raw(raw_bytes.clone().into());
    assert_eq!(value, CoseHeaderValue::Raw(raw_bytes.into()));
}
...
fn test_header_map_encode_multiple_entries() {
    ...
    map.insert(
        CoseHeaderLabel::Int(4),
        CoseHeaderValue::Bytes(vec![0xaa, 0xbb].into()),
    );
...
fn test_header_map_encode_decode_bool_values() {
    ...
    assert_eq!(
        decoded.get(&CoseHeaderLabel::Int(41)),
...
fn test_header_map_encode_decode_null_undefined() {
    ...
    assert_eq!(
        decoded.get(&CoseHeaderLabel::Int(51)),
...
fn test_header_map_crit_filters_non_label_values() {
    ...
    map.insert(
        ...
        CoseHeaderValue::Array(vec![
            ...
            CoseHeaderValue::Bytes(vec![1, 2, 3].into()),
            CoseHeaderValue::Text("ext".to_string().into()),
        ]),
    );
