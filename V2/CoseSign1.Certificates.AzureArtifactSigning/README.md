...
Azure Artifact Signing integration for COSE Sign1 message signing.
...
Integrates with Microsoft Azure Artifact Signing service for cloud-based, HSM-backed code signing. Provides enterprise-grade security with FIPS 140-2 Level 3 compliance.
...
using var signingService = new AzureArtifactSigningService(signContext);
...
Certificate source for Azure Artifact Signing certificates:

```csharp
public class AzureArtifactSigningCertificateSource : ICertificateSource
{
    // Provides certificate chain from Azure Artifact Signing
}
```

### AzureArtifactSigningDidX509

DID:x509 integration for Azure certificates:

```csharp
// Generate DID:x509 from Azure certificate
var did = await AzureArtifactSigningDidX509.CreateDidAsync(signingService);
...
1. **Azure Subscription** with Azure Artifact Signing access
2. **Artifact Signing Account** in Azure
3. **Certificate Profile** configured

### Create Artifact Signing Account
...
1. Navigate to your Artifact Signing Account
...
Azure Artifact Signing with CWT claims for SCITT:
...
Azure Artifact Signing supports:
...
try
{
    using var service = new AzureArtifactSigningService(signContext);
    // ... signing operations
}
