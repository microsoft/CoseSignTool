...
use cose_sign1_transparent_mst_ffi::{
    ...
    cose_mst_trust_options_t, cose_sign1_mst_create_entry, cose_sign1_mst_get_entry_statement,
...
fn make_transparent_null_cose_bytes() {
    ...
    let mut out_len: usize = 0;
    let status =
        cose_sign1_mst_make_transparent(client, ptr::null(), 0, &mut out_bytes, &mut out_len);
    assert_ne!(status, cose_status_t::COSE_OK);
    unsafe { cose_mst_client_free(client) };
}
...
fn create_entry_null_cose_bytes() {
    ...
    let mut op_id: *mut c_char = ptr::null_mut();
    let mut entry_id: *mut c_char = ptr::null_mut();
    let status = cose_sign1_mst_create_entry(client, ptr::null(), 0, &mut op_id, &mut entry_id);
    assert_ne!(status, cose_status_t::COSE_OK);
    unsafe { cose_mst_client_free(client) };
}
...
fn get_entry_statement_null_entry_id() {
    ...
    let mut out_len: usize = 0;
    let status =
        cose_sign1_mst_get_entry_statement(client, ptr::null(), &mut out_bytes, &mut out_len);
    assert_ne!(status, cose_status_t::COSE_OK);
    unsafe { cose_mst_client_free(client) };
}
...
fn policy_issuer_eq_null_string() {
    ...
    assert_ne!(
        cose_sign1_mst_trust_policy_builder_require_receipt_issuer_eq(&mut *pb, ptr::null()),
        cose_status_t::COSE_OK
    );
}
...
fn with_mst_pack_ex_null_builder() {
    let status = cose_sign1_validator_builder_with_mst_pack_ex(ptr::null_mut(), ptr::null());
    assert_ne!(status, cose_status_t::COSE_OK);
}
...
fn client_new_no_api_version_no_key() {
    let endpoint = CString::new("https://mst.example.com").unwrap();
    let mut client: *mut MstClientHandle = ptr::null_mut();
    let status = cose_mst_client_new(endpoint.as_ptr(), ptr::null(), ptr::null(), &mut client);
    assert_eq!(status, cose_status_t::COSE_OK);
