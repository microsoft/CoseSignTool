...
/// # Safety
///
/// - `s` must be a valid pointer from `cose_cert_local_last_error_message_utf8()`, or null (no-op).
/// - Must not be called twice on the same string (double-free is UB).
...
pub unsafe extern "C" fn cose_cert_local_string_free(s: *mut c_char) {
    ...
    // SAFETY: String was allocated via `CString::into_raw` in `take_last_error_ptr`; reclaiming it.
    unsafe {
...
/// # Safety
///
/// - `factory` must be a valid handle returned by `cose_cert_local_factory_new`, or null (no-op).
/// - Must not be called twice on the same handle (double-free is UB).
#[no_mangle]
#[cfg_attr(coverage_nightly, coverage(off))]
pub extern "C" fn cose_cert_local_factory_free(factory: *mut cose_cert_local_factory_t) {
    ...
    // SAFETY: Handle was allocated via `Box::into_raw` in `cose_cert_local_factory_new`; reclaiming it.
    unsafe {
...
fn string_from_ptr(arg_name: &'static str, s: *const c_char) -> Result<String, anyhow::Error> {
    ...
    // SAFETY: Null checked above; caller guarantees `s` points to a valid C string per FFI contract.
...
pub extern "C" fn cose_cert_local_factory_create_cert(
    ...
    with_catch_unwind(|| {
        ...
        // SAFETY: Output pointers are non-null (checked above); writing Box::into_raw results for caller to free.
        unsafe {
...
/// # Safety
///
/// - `chain_factory` must be a valid handle returned by `cose_cert_local_chain_new`, or null (no-op).
/// - Must not be called twice on the same handle (double-free is UB).
#[no_mangle]
#[cfg_attr(coverage_nightly, coverage(off))]
pub extern "C" fn cose_cert_local_chain_free(chain_factory: *mut cose_cert_local_chain_t) {
    ...
    // SAFETY: Handle was allocated via `Box::into_raw` in `cose_cert_local_chain_new`; reclaiming it.
    unsafe {
...
pub extern "C" fn cose_cert_local_chain_create(
    ...
    with_catch_unwind(|| {
        ...
        for cert in chain {
            ...
            // SAFETY: `cert_box_ptr` is valid; getting the data pointer from the owned boxed slice.
            ...
            if let Some(key_der) = cert.private_key_der {
                ...
                // SAFETY: `key_box_ptr` is valid; getting the data pointer from the owned boxed slice.
...
pub extern "C" fn cose_cert_local_load_pem(
    ...
    with_catch_unwind(|| {
        ...
        if let Some(key_der) = cert.private_key_der {
            ...
            // SAFETY: Output pointers are non-null (checked above); writing null/zero for absent key.
            unsafe {
...
/// # Safety
///
/// - `ptr` must be a valid pointer to a byte buffer allocated by this library, or null (no-op).
/// - `len` must be the length originally returned alongside `ptr`.
/// - Must not be called twice on the same pointer (double-free is UB).
...
pub unsafe extern "C" fn cose_cert_local_bytes_free(ptr: *mut u8, len: usize) {
    ...
    // SAFETY: `ptr`/`len` were produced by `Box::into_raw` of a boxed slice in this library; reclaiming it.
    unsafe {
...
/// # Safety
///
/// - `ptr` must be a valid pointer to a pointer array allocated by this library, or null (no-op).
/// - `len` must be the length originally returned alongside `ptr`.
/// - Must not be called twice on the same pointer (double-free is UB).
#[no_mangle]
#[cfg_attr(coverage_nightly, coverage(off))]
...
/// # Safety
///
/// - `ptr` must be a valid pointer to a lengths array allocated by this library, or null (no-op).
/// - `len` must be the length originally returned alongside `ptr`.
/// - Must not be called twice on the same pointer (double-free is UB).
#[no_mangle]
#[cfg_attr(coverage_nightly, coverage(off))]
