...
fn display_array_multiple() {
    let val = CoseHeaderValue::Array(vec![
        ...
        CoseHeaderValue::Text("two".to_string().into()),
        CoseHeaderValue::Bytes(vec![3].into()),
    ]);
...
fn encode_decode_array_of_ints() {
    ...
    map.insert(
        ...
        CoseHeaderValue::Array(vec![CoseHeaderValue::Int(10), CoseHeaderValue::Int(-20)]),
    );
    let bytes = map.encode().unwrap();
    let decoded = CoseHeaderMap::decode(&bytes).unwrap();
...
fn encode_decode_nested_map_value() {
    let inner = vec![
        ...
        (
            ...
            CoseHeaderValue::Bytes(vec![1].into()),
        ),
    ];
    let mut map = CoseHeaderMap::new();
...
fn encode_decode_raw_passthrough() {
    ...
    map.insert(
        ...
        CoseHeaderValue::Raw(raw_cbor.clone().into()),
    );
    let bytes = map.encode().unwrap();
    let decoded = CoseHeaderMap::decode(&bytes).unwrap();
...
fn decode_text_string_value_via_cbor() {
    ...
    assert_eq!(
        ...
        Some(&CoseHeaderValue::Text("value-text".to_string().into()))
    );
}
...
fn encode_decode_multi_entry_map() {
    ...
    map.insert(
        ...
        CoseHeaderValue::Bytes(b"kid".to_vec().into()),
    );
    map.insert(
        CoseHeaderLabel::Text("x".to_string()),
...
fn decode_map_value_containing_map() {
    let inner = CoseHeaderValue::Map(vec![(CoseHeaderLabel::Int(10), CoseHeaderValue::Int(20))]);
    let outer = CoseHeaderValue::Map(vec![(CoseHeaderLabel::Int(1), inner)]);
    let mut map = CoseHeaderMap::new();
...
fn decode_tagged_value_from_cbor() {
    ...
    if let Some(CoseHeaderValue::Tagged(tag, inner)) = decoded.get(&CoseHeaderLabel::Int(500)) {
        assert_eq!(*tag, 18);
        assert_eq!(**inner, CoseHeaderValue::Bytes(vec![0xAB, 0xCD].into()));
    } else {
        panic!("expected Tagged");
    }
}
