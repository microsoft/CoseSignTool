...
// Tests for pure logic in ats_crypto_signer.rs
...
fn test_ats_crypto_signer_algorithm_name_patterns() {
    // Test algorithm name patterns recognized by AtsCryptoSigner
    let algorithms = vec![
        ("RS256", "SHA-256", 32),
        ("RS384", "SHA-384", 48),
        ("RS512", "SHA-512", 64),
...
fn test_ats_crypto_signer_error_conversion() {
    // Test error conversion from AtsError to CryptoError
    let ats_error_msg = "ATS sign operation failed";
    let crypto_error = format!("SigningFailed: {}", ats_error_msg);
    
    assert!(crypto_error.contains("SigningFailed"));
    assert!(crypto_error.contains("ATS sign operation failed"));
}
...
fn test_ats_crypto_signer_construction_parameters() {
    // Test AtsCryptoSigner construction parameter validation
