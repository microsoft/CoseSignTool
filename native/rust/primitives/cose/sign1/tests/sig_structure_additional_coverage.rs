...
fn test_sig_structure_hasher_double_init_error() {
    ...
    hasher
        .init(protected, None, payload_len)
        .expect("first init");
...
fn test_hash_sig_structure_streaming() {
    ...
    let result = hash_sig_structure_streaming(hasher, protected, external_aad, payload)
...
fn test_hash_streaming_len_failure() {
    ...
    let mut payload =
        MockSizedRead::new(payload_data.to_vec(), payload_data.len() as u64).with_len_failure();
    let mut hasher = Vec::<u8>::new();
    ...
    assert!(result
        .unwrap_err()
        .to_string()
        .contains("failed to get payload length"));
}
...
fn test_hash_streaming_read_failure() {
    ...
    let mut payload =
        MockSizedRead::new(payload_data.to_vec(), payload_data.len() as u64).with_read_failure();
    let mut hasher = Vec::<u8>::new();
...
fn test_stream_sig_structure() {
    ...
    let bytes_written = stream_sig_structure(&mut output, protected, external_aad, payload)
...
fn test_stream_sig_structure_empty_payload() {
    ...
    let bytes_written = stream_sig_structure(&mut output, protected, None, payload)
...
fn test_stream_sig_structure_large_external_aad() {
    ...
    let bytes_written =
        stream_sig_structure(&mut output, protected, Some(&large_aad), payload_reader)
...
fn test_stream_exactly_85kb_payload() {
    ...
    let bytes_written = stream_sig_structure(&mut output, protected, None, payload_reader)
...
fn test_sig_structure_hasher_update_write_error() {
    ...
    let mut hasher = SigStructureHasher::new(FailOnSecondWrite { write_count: 0 });
...
fn test_stream_sig_structure_external_aad_variations() {
    ...
    stream_sig_structure(&mut output1, protected, None, payload1)
    ...
    stream_sig_structure(&mut output2, protected, Some(empty_aad), payload2)
...
fn test_stream_sig_structure_very_small_chunk_size() {
    ...
    let bytes_written =
        stream_sig_structure_chunked(&mut output, protected, None, &mut payload_reader, 1)
...
fn test_stream_sig_structure_all_params() {
    ...
    let bytes_written =
        stream_sig_structure(&mut output, protected, Some(external_aad), payload_reader)
...
fn test_hash_sig_structure_streaming_all_params() {
    ...
    let result =
        hash_sig_structure_streaming(hasher, protected, Some(external_aad), payload_reader)
...
fn test_sig_structure_hasher_external_aad_variations() {
    ...
    hasher1
        .init(b"\xa0", None, 10)
        .expect("should init with None");

    // Test with Some(&[])
    let mut hasher2 = SigStructureHasher::new(Vec::<u8>::new());
    hasher2
        .init(b"\xa0", Some(b""), 10)
        .expect("should init with empty");

    // Test with Some(data)
    let mut hasher3 = SigStructureHasher::new(Vec::<u8>::new());
    hasher3
        .init(b"\xa0", Some(b"data"), 10)
        .expect("should init with data");
...
fn test_stream_sig_structure_write_failure_mid_payload() {
    ...
    let result = stream_sig_structure_chunked(&mut output, b"\xa0", None, &mut payload, 5);
...
fn test_hash_streaming_read_failure_mid_payload() {
    ...
    let result = hash_sig_structure_streaming_chunked(&mut hasher, b"\xa0", None, &mut payload, 2);
