...
fn test_claims_roundtrip_with_all_fields() {
    ...
    unsafe {
        ...
        assert_eq!(
            cose_cwt_claims_set_issued_at(handle, 1640995200, &mut error),
            COSE_CWT_OK
        );
        assert_eq!(
            cose_cwt_claims_set_not_before(handle, 1640995200, &mut error),
            COSE_CWT_OK
        );
        assert_eq!(
            cose_cwt_claims_set_expiration(handle, 1672531200, &mut error),
            COSE_CWT_OK
        );
        ...
        assert_eq!(result, COSE_CWT_OK);
        assert_eq!(
            CStr::from_ptr(subject_ptr).to_str().unwrap(),
            "test-subject"
        );
