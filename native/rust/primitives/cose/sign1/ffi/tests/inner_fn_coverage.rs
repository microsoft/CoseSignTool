...
use cose_sign1_primitives_ffi::message::{
    message_alg_inner, message_is_detached_inner, message_parse_inner, message_payload_inner,
...
use cose_sign1_primitives_ffi::{
    ...
    headermap_get_int_inner, headermap_get_text_inner, headermap_len_inner, key_algorithm_inner,
    key_type_inner, message_protected_headers_inner, message_unprotected_headers_inner,
};
...
impl cose_sign1_primitives::CryptoVerifier for MockVerifier {
    ...
    fn verify(
        ...
        _data: &[u8],
        _signature: &[u8],
    ) -> Result<bool, cose_sign1_primitives::CryptoError> {
...
fn inner_verify_with_key_returns_ok() {
    ...
    let mut err: *mut CoseSign1ErrorHandle = ptr::null_mut();
    let rc = message_verify_inner(msg, key_handle, ptr::null(), 0, &mut verified, &mut err);
    assert_eq!(rc, 0);
...
fn inner_verify_null_out_verified() {
    ...
    let msg = parse_msg(&data);
    let mut err: *mut CoseSign1ErrorHandle = ptr::null_mut();
    let rc = message_verify_inner(msg, ptr::null(), ptr::null(), 0, ptr::null_mut(), &mut err);
    assert!(rc < 0);
    free_error(err);
    free_msg(msg);
}
...
fn inner_verify_null_key() {
    ...
    let mut verified = false;
    let mut err: *mut CoseSign1ErrorHandle = ptr::null_mut();
    let rc = message_verify_inner(msg, ptr::null(), ptr::null(), 0, &mut verified, &mut err);
    assert!(rc < 0);
    free_error(err);
    free_msg(msg);
}
...
fn inner_key_type_with_mock() {
    ...
    let s = unsafe { CStr::from_ptr(key_type) }
        .to_string_lossy()
        .to_string();
