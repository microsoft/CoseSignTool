...
fn create_mock_cert_der() -> Vec<u8> {
    ...
    vec![
        ...
        0x02, 0x08, 0x12, 0x34, 0x56, 0x78, 0x9A, 0xBC, 0xDE,
        0xF0, // serial
...
fn test_compute_thumbprint_sha384() {
    ...
    assert_ne!(
        thumbprint.len(),
        sha256_thumbprint.len(),
        "SHA-384 and SHA-256 should produce different lengths"
    );
}
...
fn test_cose_x509_thumbprint_serialize_sha384() {
    ...
    let serialized = thumbprint
        .serialize()
        .expect("Should serialize SHA-384 successfully");
    assert!(
        !serialized.is_empty(),
        "Serialized SHA-384 data should not be empty"
    );
}
...
fn test_cose_x509_thumbprint_serialize_sha512() {
    ...
    let serialized = thumbprint
        .serialize()
        .expect("Should serialize SHA-512 successfully");
    assert!(
        !serialized.is_empty(),
        "Serialized SHA-512 data should not be empty"
    );
}
...
fn test_cose_x509_thumbprint_deserialize_all_algorithms() {
    ...
    for algorithm in [
        ...
        let serialized = original.serialize().expect("Should serialize");
        let deserialized =
            CoseX509Thumbprint::deserialize(&serialized).expect("Should deserialize");
...
fn test_cose_x509_thumbprint_deserialize_not_array() {
    ...
    assert!(
        error_msg.contains("first level must be an array"),
...
fn test_cose_x509_thumbprint_deserialize_wrong_array_length() {
    ...
    let error_msg = result.unwrap_err().to_string();
    assert!(
        error_msg.contains("2 element array"),
        "Should mention 2 element requirement"
    );
}
...
fn test_cose_x509_thumbprint_deserialize_first_not_int() {
    ...
    assert!(
        result.is_err(),
        "Should fail when first element is not integer"
    );
    let error_msg = result.unwrap_err().to_string();
    assert!(
        error_msg.contains("first member must be integer"),
        "Should mention integer requirement"
    );
}
...
fn test_cose_x509_thumbprint_deserialize_unsupported_algorithm() {
    ...
    let error_msg = result.unwrap_err().to_string();
    assert!(
        error_msg.contains("Unsupported thumbprint hash algorithm"),
...
fn test_cose_x509_thumbprint_deserialize_second_not_bstr() {
    ...
    assert!(
        result.is_err(),
        "Should fail when second element is not byte string"
    );
    let error_msg = result.unwrap_err().to_string();
    assert!(
        error_msg.contains("second member must be ByteString"),
        "Should mention byte string requirement"
    );
}
...
fn test_cose_x509_thumbprint_matches_same_cert() {
    ...
    let matches = thumbprint
        .matches(&cert_der)
        .expect("Should check match successfully");
...
fn test_cose_x509_thumbprint_matches_unsupported_hash() {
    ...
    let error_msg = result.unwrap_err().to_string();
    assert!(
        error_msg.contains("Unsupported hash ID"),
        "Should mention unsupported hash ID"
    );
}
...
fn test_cose_x509_thumbprint_matches_different_algorithms() {
    ...
    assert!(
        sha256_thumbprint.matches(&cert_der).unwrap(),
        "SHA-256 thumbprint should match"
    );
    assert!(
        sha384_thumbprint.matches(&cert_der).unwrap(),
        "SHA-384 thumbprint should match"
    );
    assert!(
        sha512_thumbprint.matches(&cert_der).unwrap(),
        "SHA-512 thumbprint should match"
    );
...
fn test_cose_x509_thumbprint_empty_certificate() {
    ...
    assert!(
        thumbprint.matches(&empty_cert).unwrap(),
        "Should match empty certificate"
    );
}
...
fn test_cose_x509_thumbprint_large_certificate() {
    ...
    assert_eq!(thumbprint.thumbprint.len(), 32);
    assert!(
        thumbprint.matches(&large_cert).unwrap(),
        "Should match large certificate"
    );
}
