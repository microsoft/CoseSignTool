...
use cosesign1_abstractions::{ParsedCoseSign1, ValidationFailure, ValidationResult};
use std::io::Read;

use crate::{CoseHashAlgorithm, ReadSeek, SignatureVerificationSettings, VerifyOptions, VerificationSettings};
...
impl CoseSign1 {
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, String> {
        let parsed = crate::parse_cose_sign1(bytes)?;
        Ok(Self {
            bytes: bytes.to_vec(),
            parsed,
        })
    ...
    pub fn verify_signature_with_payload_reader(
                    ...
                    Ok(false) => {
                        return ValidationResult::failure(
                            ...
                            vec![ValidationFailure {
                                message:
                                    "payload does not match embedded COSE Hash Envelope digest".to_string(),
        ...
        let mut sig = if self.parsed.payload.is_none() {
            crate::verify_parsed_cose_sign1_detached_payload_reader(
                "Signature",
                &self.parsed,
                payload_reader,
                &opts,
            ...
            crate::verify_parsed_cose_sign1(
                "Signature",
                &self.parsed,
    ...
    pub fn verify(
        ...
        let sig = if settings.require_cose_signature {
            Some(self.verify_signature_with_settings(
                payload_to_verify,
                public_key_bytes,
                &settings.signature,
            ))
    ...
    fn verify_signature_impl(
        ...
        let mut sig = crate::verify_parsed_cose_sign1("Signature", &self.parsed, external, &opts);
