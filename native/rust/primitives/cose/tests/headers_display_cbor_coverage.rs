...
fn test_header_value_display() {
    ...
    // Test Bytes display
    let bytes_val = CoseHeaderValue::Bytes(vec![1, 2, 3, 4, 5].into());
    assert_eq!(format!("{}", bytes_val), "bytes(5)");

    // Test Text display
    let text_val = CoseHeaderValue::Text("hello world".to_string().into());
    ...
    // Test Raw display
    let raw_val = CoseHeaderValue::Raw(vec![0x01, 0x02, 0x03].into());
...
fn test_header_value_map_display() {
    let map_val = CoseHeaderValue::Map(vec![
        (
            ...
            CoseHeaderValue::Text("alg".to_string().into()),
        ),
        (
...
fn test_header_value_tagged_display() {
    let tagged_val = CoseHeaderValue::Tagged(
        18,
        Box::new(CoseHeaderValue::Text("tagged content".to_string().into())),
    );
...
fn test_cbor_roundtrip_all_header_value_types() {
    let test_values = vec![
        ...
        CoseHeaderValue::Bytes(vec![1, 2, 3, 255].into()),
        CoseHeaderValue::Text(String::new().into()),
        CoseHeaderValue::Text("test string".to_string().into()),
        CoseHeaderValue::Text("UTF-8: 测试".to_string().into()),
        CoseHeaderValue::Bool(true),
    ...
    for (i, original) in test_values.iter().enumerate() {
        ...
        let decoded_value = decoded_map
            .get(&CoseHeaderLabel::Int(i as i64))
            .expect("should find the value");

        assert_eq!(
            original, decoded_value,
...
fn test_cbor_roundtrip_complex_nested_structures() {
    ...
    let complex_array = CoseHeaderValue::Array(vec![
        CoseHeaderValue::Map(vec![
            ...
            (
                CoseHeaderLabel::Text("array".to_string()),
                CoseHeaderValue::Array(vec![
                    CoseHeaderValue::Int(1),
    ...
    let mut map = CoseHeaderMap::new();
    map.insert(
        CoseHeaderLabel::Text("complex".to_string()),
        complex_array.clone(),
    );
    ...
    let decoded_value = decoded_map
        .get(&CoseHeaderLabel::Text("complex".to_string()))
...
fn test_header_value_as_bytes_one_or_many_edge_cases() {
    // Test single bytes
    let single_bytes = CoseHeaderValue::Bytes(vec![1, 2, 3].into());
    let result = single_bytes
        .as_bytes_one_or_many()
        .expect("should extract bytes");
    assert_eq!(result, vec![vec![1, 2, 3]]);

    // Test array of bytes
    let array_bytes = CoseHeaderValue::Array(vec![
        CoseHeaderValue::Bytes(vec![1, 2].into()),
        CoseHeaderValue::Bytes(vec![3, 4, 5].into()),
        CoseHeaderValue::Bytes(vec![].into()),
    ]);
    let result = array_bytes
        .as_bytes_one_or_many()
        .expect("should extract bytes array");
    ...
    let result = mixed_array
        .as_bytes_one_or_many()
        .expect("should extract bytes from mixed array");
    ...
    let text_value = CoseHeaderValue::Text("not bytes".to_string().into());
...
fn test_content_type_edge_cases() {
    ...
    map.insert(
        CoseHeaderLabel::Int(CoseHeaderMap::CONTENT_TYPE),
        CoseHeaderValue::Int(u16::MAX as i64 + 1),
    );
    assert_eq!(map.content_type(), None);
    ...
    map.insert(
        CoseHeaderLabel::Int(CoseHeaderMap::CONTENT_TYPE),
        CoseHeaderValue::Uint(u16::MAX as u64),
    );
    assert_eq!(map.content_type(), Some(ContentType::Int(u16::MAX)));

    map.insert(
        CoseHeaderLabel::Int(CoseHeaderMap::CONTENT_TYPE),
        CoseHeaderValue::Uint(u16::MAX as u64 + 1),
    );
    assert_eq!(map.content_type(), None);
}
