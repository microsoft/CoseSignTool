...
use cose_sign1_primitives::{
    build_sig_structure, build_sig_structure_prefix, error::CoseSign1Error,
    hash_sig_structure_streaming, sized_from_bytes, sized_from_read_buffered, sized_from_reader,
    sized_from_seekable, stream_sig_structure, IntoSizedRead, SigStructureHasher, SizedRead,
    SizedReader, SizedSeekReader,
};
...
fn test_hash_sig_structure_streaming() {
    ...
    let hasher =
        hash_sig_structure_streaming(MockHasher::new(), protected, external_aad, payload).unwrap();
...
fn test_stream_sig_structure() {
    ...
    let bytes_written =
        stream_sig_structure(&mut output, protected, external_aad, payload).unwrap();
...
fn test_hash_sig_structure_streaming_len_error() {
    ...
    let result = hash_sig_structure_streaming(MockHasher::new(), b"protected", None, payload);
...
fn test_hash_sig_structure_streaming_length_mismatch() {
    ...
    assert!(result.is_err());
    match result.unwrap_err() {
        CoseSign1Error::PayloadError(cose_sign1_primitives::PayloadError::LengthMismatch {
            expected,
            actual,
        }) => {
