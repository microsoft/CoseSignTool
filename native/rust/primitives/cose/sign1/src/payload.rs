...
///
/// Stores the entire payload in memory behind an [`Arc`], so [`open()`](StreamingPayload::open)
/// is a cheap pointer copy instead of cloning the full buffer.
#[derive(Clone, Debug)]
...
impl MemoryPayload {
    ...
    /// Consumes the payload and returns the underlying data as a `Vec<u8>`.
...
impl StreamingPayload for MemoryPayload {
    ...
    fn open(&self) -> Result<Box<dyn SizedRead + Send>, PayloadError> {
        // Arc clone is a cheap atomic reference-count increment.
