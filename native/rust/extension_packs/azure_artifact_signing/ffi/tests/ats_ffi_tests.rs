...
use cose_sign1_azure_artifact_signing_ffi::{
    cose_ats_trust_options_t, cose_sign1_ats_abi_version,
    cose_sign1_validator_builder_with_ats_pack, cose_sign1_validator_builder_with_ats_pack_ex,
};
...
fn with_ats_pack_ex_null_builder() {
    let status =
        cose_sign1_validator_builder_with_ats_pack_ex(std::ptr::null_mut(), std::ptr::null());
    assert_ne!(status, cose_status_t::COSE_OK);
}
...
fn with_ats_pack_ex_null_options() {
    let mut builder = make_builder();
    let status = cose_sign1_validator_builder_with_ats_pack_ex(&mut *builder, std::ptr::null());
    assert_eq!(status, cose_status_t::COSE_OK);
}
...
fn with_ats_pack_ex_with_options() {
    ...
    let mut builder = make_builder();
    let status = cose_sign1_validator_builder_with_ats_pack_ex(&mut *builder, &opts);
    assert_eq!(status, cose_status_t::COSE_OK);
}
