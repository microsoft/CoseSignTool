...
[TestFixture]
[System.Runtime.Versioning.RequiresPreviewFeatures("Uses preview cryptography APIs.")]
public class TransparencyReceiptExtensionsTests
{
    private static CoseSign1Message CreateTestMessage()
    {
        using X509Certificate2 cert = TestCertificateUtils.CreateCertificate("ReceiptExtTest", useEcc: true);
    ...
    [Test]
    public void TryGetReceipts_WithNoReceipts_ReturnsFalse()
    {
        // Arrange
    ...
    [Test]
    public void TryGetReceipts_WithNullMessage_ThrowsArgumentNullException()
    {
        // Act & Assert
        Assert.Throws<ArgumentNullException>(() =>
        {
            CoseSign1Message? nullMessage = null;
            nullMessage!.TryGetReceipts(out _);
        });
    }
    ...
    public void AddReceipts_AddsReceiptsSuccessfully()
    {
        // Arrange
        CoseSign1Message message = CreateTestMessage();
        List<byte[]> receipts = new() { new byte[] { 1, 2, 3 } };

        // Act
        message.AddReceipts(receipts);

        // Assert
        bool result = message.TryGetReceipts(out List<byte[]>? extracted);
        Assert.That(result, Is.True);
        Assert.That(extracted, Has.Count.EqualTo(1));
        Assert.That(extracted![0], Is.EquivalentTo(new byte[] { 1, 2, 3 }));
    }

    [Test]
    public void AddReceipts_WithExistingReceipts_MergesSuccessfully()
    {
        // Arrange
        CoseSign1Message message = CreateTestMessage();
        ...
        // Act
        message.AddReceipts(new List<byte[]> { secondReceipt });

        // Assert
        bool result = message.TryGetReceipts(out List<byte[]>? extracted);
        Assert.That(result, Is.True);
        Assert.That(extracted, Has.Count.EqualTo(2));
        Assert.That(extracted![0], Is.EquivalentTo(firstReceipt));
        Assert.That(extracted[1], Is.EquivalentTo(secondReceipt));
    }
    ...
    public void AddReceipts_DeduplicatesReceipts_ByContent()
    {
        ...
        // Assert - should be de-duplicated
        bool result = message.TryGetReceipts(out List<byte[]>? extracted);
        Assert.That(result, Is.True);
        Assert.That(extracted, Has.Count.EqualTo(1));
        Assert.That(extracted![0], Is.EquivalentTo(receipt));
    }

    [Test]
    public void AddReceipts_WithNullMessage_ThrowsArgumentNullException()
    {
        // Act & Assert
        Assert.Throws<ArgumentNullException>(() =>
        {
            CoseSign1Message? nullMessage = null;
            nullMessage!.AddReceipts(new List<byte[]> { new byte[] { 1 } });
        });
    }

    [Test]
    public void AddReceipts_WithNullReceipts_ThrowsArgumentNullException()
    {
        // Arrange
        CoseSign1Message message = CreateTestMessage();
