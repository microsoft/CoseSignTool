...
impl CoseSign1Validator {
    ...
    fn run_signature_stage(
        ...
        if message.payload().is_none() {
            ...
            if let Payload::Streaming(streaming) = p {
                ...
                if len > Self::LARGE_STREAM_THRESHOLD {
                    ...
                    let mut verifying_ctx = match cose_key.verify_init(message.signature()) {
                        Ok(ctx) => ctx,
                        Err(e) => {
                            return ValidationResult::failure_message(
                                Self::STAGE_NAME_SIGNATURE,
        ...
        // For embedded payloads, borrow directly from the message's backing Arc
        // (zero-copy). For detached payloads, read into a new buffer.
        let detached_buf: Arc<[u8]>;
        let payload_ref: &[u8] = if let Some(embedded) = message.payload() {
            embedded
        } else {
