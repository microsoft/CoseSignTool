...
fn test_header_value_accessor_methods() {
    // Test as_bytes
    let bytes_val = CoseHeaderValue::Bytes(b"test".to_vec().into());
    ...
    // Test as_str
    let text_val = CoseHeaderValue::Text("hello".to_string().into());
...
fn test_header_value_as_bytes_one_or_many() {
    // Single bytes value
    let single = CoseHeaderValue::Bytes(b"cert1".to_vec().into());
    assert_eq!(single.as_bytes_one_or_many(), Some(vec![b"cert1".to_vec()]));

    // Array of bytes values
    let array = CoseHeaderValue::Array(vec![
        CoseHeaderValue::Bytes(b"cert1".to_vec().into()),
        CoseHeaderValue::Bytes(b"cert2".to_vec().into()),
    ]);
    assert_eq!(
        array.as_bytes_one_or_many(),
        Some(vec![b"cert1".to_vec(), b"cert2".to_vec()])
    );

    // Array with mixed types (returns only the bytes elements)
    let mixed_array = CoseHeaderValue::Array(vec![
        CoseHeaderValue::Bytes(b"cert".to_vec().into()),
        CoseHeaderValue::Int(42),
    ]);
    assert_eq!(
        mixed_array.as_bytes_one_or_many(),
        Some(vec![b"cert".to_vec()])
    );
...
fn test_header_map_content_type_operations() {
    ...
    map.insert(
        CoseHeaderLabel::Int(CoseHeaderMap::CONTENT_TYPE),
        CoseHeaderValue::Bytes(b"invalid".to_vec().into()),
    );
    assert_eq!(map.content_type(), None);
}
...
fn test_header_map_critical_headers() {
    ...
    map.insert(
        ...
        CoseHeaderValue::Array(vec![
            CoseHeaderValue::Int(1),
            CoseHeaderValue::Bytes(b"invalid".to_vec().into()), // Invalid - not int or text
        ]),
    );
...
fn test_header_map_basic_operations() {
    ...
    let value = CoseHeaderValue::Text("test".to_string().into());
...
fn test_header_map_encode_all_value_types() {
    ...
    map.insert(
        ...
        CoseHeaderValue::Bytes(b"bytes".to_vec().into()),
    );
    map.insert(
        CoseHeaderLabel::Int(4),
...
fn test_protected_header_operations() {
    ...
    assert_eq!(
        protected.get(&CoseHeaderLabel::Int(1)),
        Some(&CoseHeaderValue::Int(-7))
    );
