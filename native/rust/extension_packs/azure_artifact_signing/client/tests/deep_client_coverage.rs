...
fn error_is_std_error() {
    let e: Box<dyn std::error::Error> = Box::new(AasClientError::HttpError("test".to_string()));
...
fn build_eku_request_basic() {
    let endpoint = Url::parse("https://eus.codesigning.azure.net").unwrap();
    let request = build_eku_request(&endpoint, "2022-06-15-preview", "acct", "prof").unwrap();
...
fn build_root_certificate_request_basic() {
    let endpoint = Url::parse("https://eus.codesigning.azure.net").unwrap();
    let request =
        build_root_certificate_request(&endpoint, "2022-06-15-preview", "acct", "prof").unwrap();
...
fn build_certificate_chain_request_basic() {
    let endpoint = Url::parse("https://eus.codesigning.azure.net").unwrap();
    let request =
        build_certificate_chain_request(&endpoint, "2022-06-15-preview", "acct", "prof").unwrap();
