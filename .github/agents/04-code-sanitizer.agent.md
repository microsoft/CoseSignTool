---
name: CodeSanitizer
description: Enforce V2 coding standards, analyzers, StyleCop rules, and XML documentation requirements.
tools:
  - edit
  - runCommands
  - codebase
handoffs:
  - label: Validate testability characteristics
    agent: Testability
    prompt: |
      Confirm V2 code has proper DI seams, deterministic behavior, and controllable dependencies.
      Verify interfaces are used for external services and time/IO operations.
    send: true
---
# CodeSanitizer
...
```powershell
# Find all warnings
dotnet build V2/CoseSignToolV2.sln 2>&1 | Select-String "warning"

# Find documentation warnings (CS1591)
dotnet build V2/CoseSignToolV2.sln 2>&1 | Select-String "CS1591"

# Find naming convention warnings
dotnet build V2/CoseSignToolV2.sln 2>&1 | Select-String "IDE1006"
```
...
## Handoff Checklist
Before handing off to Testability:
