...
use cose_sign1_validation_ffi::{
    cose_sign1_validator_builder_new, cose_sign1_validator_builder_t, cose_status_t,
};
...
fn test_trust_plan_builder_pack_queries_edge_cases() {
    ...
    let status = unsafe { cose_sign1_trust_plan_builder_pack_count(plan_builder, ptr::null_mut()) };
    assert_ne!(status, cose_status_t::COSE_OK);

    // Test pack count with null plan builder
    let mut count = 0usize;
    let status = unsafe { cose_sign1_trust_plan_builder_pack_count(ptr::null(), &mut count) };
    assert_ne!(status, cose_status_t::COSE_OK);

    // Test pack name with invalid index
    let pack_name_ptr = unsafe { cose_sign1_trust_plan_builder_pack_name_utf8(plan_builder, 999) };
    ...
    let status = unsafe {
        cose_sign1_trust_plan_builder_pack_has_default_plan(plan_builder, 0, ptr::null_mut())
    };
    assert_ne!(status, cose_status_t::COSE_OK);
...
fn test_validator_builder_with_compiled_trust_plan() {
    ...
    let status = unsafe {
        cose_sign1_validator_builder_with_compiled_trust_plan(ptr::null_mut(), compiled_plan)
    };
    assert_ne!(status, cose_status_t::COSE_OK);

    // Test with null compiled plan
    let status = unsafe {
        cose_sign1_validator_builder_with_compiled_trust_plan(validator_builder, ptr::null_mut())
    };
    assert_ne!(status, cose_status_t::COSE_OK);
...
fn test_clear_selected_plans_edge_cases() {
    ...
    let status = unsafe { cose_sign1_trust_plan_builder_clear_selected_plans(ptr::null_mut()) };
    assert_ne!(status, cose_status_t::COSE_OK);
