...
fn base_url_without_trailing_slash() {
    ...
    assert_eq!(
        opts.base_url(),
        "https://ats.example.com/codesigningaccounts/acct/certificateprofiles/prof"
    );
}
...
fn error_display_all_variants() {
    assert_eq!(
        format!("{}", AasClientError::HttpError("timeout".into())),
        "HTTP error: timeout"
    );
    assert_eq!(
        format!(
            "{}",
            AasClientError::AuthenticationFailed("bad token".into())
        ),
    ...
    assert_eq!(
        format!(
            "{}",
            AasClientError::DeserializationError("bad json".into())
        ),
        "Deserialization error: bad json"
    );
    assert_eq!(
        format!("{}", AasClientError::InvalidConfiguration("missing".into())),
        "Invalid configuration: missing"
    );
    assert_eq!(
        format!(
            "{}",
            AasClientError::CertificateChainNotAvailable("none".into())
        ),
        "Certificate chain not available: none"
    );
    assert_eq!(
        format!("{}", AasClientError::SignFailed("err".into())),
        "Sign failed: err"
    );
    assert_eq!(
        format!(
            "{}",
            AasClientError::OperationTimeout {
                operation_id: "op1".into()
            }
        ),
        "Operation op1 timed out"
    );
    assert_eq!(
        format!(
            "{}",
            AasClientError::OperationFailed {
    ...
    let with_target = AasClientError::ServiceError {
        code: "E01".into(),
        message: "bad".into(),
        target: Some("res".into()),
    };
    assert!(format!("{}", with_target).contains("(target: res)"));
    let no_target = AasClientError::ServiceError {
        code: "E01".into(),
        message: "bad".into(),
        target: None,
    };
