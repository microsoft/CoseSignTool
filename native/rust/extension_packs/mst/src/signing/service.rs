...
use cose_sign1_signing::transparency::{
    extract_receipts, TransparencyError, TransparencyProvider, TransparencyValidationResult,
};
    ...
    fn verify_transparency_proof(
        ...
        if receipts.is_empty() {
            return Ok(TransparencyValidationResult::failure(
                self.provider_name(),
                vec!["No MST receipts found in header 394".into()],
            ));
        }
        ...
        Ok(TransparencyValidationResult::failure(
            self.provider_name(),
            vec!["No valid MST receipts found".into()],
        ))
    }
}
